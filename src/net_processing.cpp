...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::INV)
    {
        ...
        if (pfrom->fWhitelisted && gArgs.GetArg("-whitelistrelay", DEFAULT_WHITELISTRELAY))
            fBlocksOnly = false;
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        if (!fRelayTxes && (!pfrom->fWhitelisted || !gArgs.GetArg("-whitelistrelay", DEFAULT_WHITELISTRELAY)))
        {
        ...
        else if (fMissingInputs)
        {
            ...
            if (!fRejectedParents) {
                ...
                unsigned int nMaxOrphanTx = (unsigned int)std::max((int64_t)0, int64_t(gArgs.GetArg("-maxorphantx", DEFAULT_MAX_ORPHAN_TRANSACTIONS)));
            ...
            if (pfrom->fWhitelisted && gArgs.GetArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY)) {
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && gArgs.GetArg("-feefilter", DEFAULT_FEEFILTER) &&
            !(pto->fWhitelisted && gArgs.GetArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY))) {
