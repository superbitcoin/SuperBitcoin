...
{
    size_t max_extra_txn = gArgs.GetArg<uint32_t>("-blockreconstructionextratxn", DEFAULT_BLOCK_RECONSTRUCTION_EXTRA_TXN);
    if (max_extra_txn <= 0)
        return;
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        else if (fMissingInputs)
        {
            ...
            if (!fRejectedParents) {
                ...
                unsigned int nMaxOrphanTx = (unsigned int)std::max((int64_t)0, int64_t(gArgs.GetArg<uint32_t>("-maxorphantx", DEFAULT_MAX_ORPHAN_TRANSACTIONS)));
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && gArgs.GetArg<bool>("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            CAmount currentFilter = mempool.GetMinFee(gArgs.GetArg<uint32_t>("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000).GetFeePerK();
