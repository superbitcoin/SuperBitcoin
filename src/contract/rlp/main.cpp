...
public:
    ...
    void output(RLP const &_d, unsigned _level = 0)
    {
        if (_d.isNull())
            ...
            if (m_prefs.hexInts)
                m_out << (m_prefs.hexPrefix ? "0x" : "")
                      << toHex(toCompactBigEndian(_d.toInt<bigint>(RLP::LaissezFaire), 1), 1);
            else
    ...
    if (mode != Mode::Create && mode != Mode::AssembleArchive)
    {
        if (encoding == Encoding::Auto)
        {
            ...
            for (char b: in)
                if (b != '\n' && b != ' ' && b != '\t')
                {
                    if (encoding == Encoding::Hex && (b < '0' || b > '9') && (b < 'a' || b > 'f') &&
                        (b < 'A' || b > 'F'))
                    {
                        //						cerr << "'" << b << "':" << (int)b << endl;
                        encoding = Encoding::Base64;
                    }
                    if (encoding == Encoding::Base64 && (b < '0' || b > '9') && (b < 'a' || b > 'z') &&
                        (b < 'A' || b > 'Z') && b != '+' && b != '/')
                    {
                        encoding = Encoding::Binary;
                        break;
            ...
            case Mode::AssembleArchive:
            {
                if (boost::filesystem::is_directory(inputFile))
                {
                    ...
                    for (string &i: otherInputs)
                    {
                        ...
                        if (!entry.count("path"))
                        {
                            std::string path = boost::filesystem::canonical(
                                    boost::filesystem::path(parsed[0])).string().substr(basePath.size());
                            if (path == "/index.html")
                                path = "/";
                            entry["path"] = path;
                        }
