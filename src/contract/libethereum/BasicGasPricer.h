...
namespace dev
{
    namespace eth
    {
        ...
        {
        public:
            explicit BasicGasPricer(u256 _weiPerRef, u256 _refsPerBlock) : m_weiPerRef(_weiPerRef),
            ...
            void setRefPrice(u256 _weiPerRef)
            {
                if ((bigint)m_refsPerBlock * _weiPerRef > std::numeric_limits<u256>::max())
                    BOOST_THROW_EXCEPTION(Overflow() << errinfo_comment(
            ...
            {
                return m_octiles[(int)_p] > 0 ? m_octiles[(int)_p] : (m_weiPerRef * m_refsPerBlock / m_gasPerBlock);
            }
