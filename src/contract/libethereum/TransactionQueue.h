...
namespace dev
{
    namespace eth
    {
        ...
        struct TransactionQueueChannel : public LogChannel
        {
        ...
        struct TransactionQueueTraceChannel : public LogChannel
        {
            static const char *name();
...
#define ctxq dev::LogOutputStream<dev::eth::TransactionQueueTraceChannel, true>()
        ...
        public:
            ...
            Limits limits() const
            {
                return Limits{m_limit, m_futureLimit};
            }
        ...
        private:
            ...
            struct UnverifiedTransaction
            {
                ...
                UnverifiedTransaction(bytesConstRef const &_t, h512 const &_nodeId) : transaction(_t.toBytes()),
                                                                                      nodeId(_nodeId)
                {
                }

                UnverifiedTransaction(UnverifiedTransaction &&_t) : transaction(std::move(_t.transaction)),
                                                                    nodeId(std::move(_t.nodeId))
                {
                }
            ...
            struct PriorityCompare
            {
                ...
                {
                    u256 const &height1 = _first.transaction.nonce() -
                                          queue.m_currentByAddressAndNonce[_first.transaction.sender()].begin()->first;
                    u256 const &height2 = _second.transaction.nonce() -
                                          queue.m_currentByAddressAndNonce[_second.transaction.sender()].begin()->first;
                    return height1 < height2 ||
                           (height1 == height2 && _first.transaction.gasPrice() > _second.transaction.gasPrice());
                }
            };
            ...
            std::atomic<bool> m_aborting = {
                    false};                                        ///< Exit condition for verifier.
        };
