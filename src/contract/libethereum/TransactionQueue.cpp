...
const char *TransactionQueueChannel::name()
{
    return EthCyan "┉┅▶";
}

const char *TransactionQueueTraceChannel::name()
{
    return EthCyan " ┅▶";
}
...
void TransactionQueue::makeCurrent_WITH_LOCK(Transaction const &_t)
{
    ...
    while (m_futureSize > m_futureLimit)
    {
        ...
        --m_futureSize;
        clog(TransactionQueueTraceChannel) << "Dropping out of bounds future transaction"
...
void TransactionQueue::enqueue(RLP const &_data, h512 const &_nodeId)
{
    bool queued = false;
    {
        ...
        for (unsigned i = 0; i < itemCount; ++i)
        {
            ...
            {
                clog(TransactionQueueChannel) << "Transaction verification queue is full. Dropping" << itemCount - i
                                              << "transactions";
                break;
            }
