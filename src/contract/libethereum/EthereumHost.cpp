...
char const *const EthereumHost::s_stateNames[static_cast<int>(SyncState::Size)] = {"NotSynced", "Idle", "Waiting",
                                                                                   "Blocks", "State", "NewBlocks"};

#if defined(_WIN32)
const char* EthereumHostTrace::name() { return EthPurple "^" EthGray "  "; }
#else

const char *EthereumHostTrace::name()
{
    return EthPurple "⧫" EthGray " ";
}
...
namespace
{
    ...
    {
    public:
        EthereumPeerObserver(BlockChainSync &_sync, RecursiveMutex &_syncMutex, TransactionQueue &_tq) : m_sync(_sync),
                                                                                                         m_syncMutex(
                                                                                                                 _syncMutex),
                                                                                                         m_tq(_tq)
        {
        ...
        void
        onPeerNewHashes(std::shared_ptr<EthereumPeer> _peer, std::vector<std::pair<h256, u256>> const &_hashes) override
        {
            RecursiveGuard l(m_syncMutex);
            try
            {
    ...
    public:
        ...
        pair<bytes, unsigned>
        blockHeaders(RLP const &_blockId, unsigned _maxHeaders, u256 _skip, bool _reverse) const override
        {
            ...
            if (_blockId.size() == 32) // block id is a hash
            {
                ...
                else if (!_reverse)
                {
                    ...
                    else if (n != 0 || blockHash == m_chain.genesisHash())
                    {
                        ...
                        blockHash = m_chain.numberHash(
                                static_cast<unsigned>(top)); // override start block hash with the hash of the top block we have
                    } else
                        blockHash = {};
                }
            ...
            auto nextHash = [this](h256 _h, unsigned _step)
            {
                ...
                const auto limitBlock = lastBlock > c_blockNumberUsageLimit ? lastBlock - c_blockNumberUsageLimit
        ...
        pair<bytes, unsigned> blockBodies(RLP const &_blockHashes) const override
        {
            ...
            if (count > 20 && n == 0)
                ...
                clog(NetMessageSummary) << n << "blocks known and returned;" << (numBodiesToSend - n)
        ...
        strings nodeData(RLP const &_dataHashes) const override
        {
            ...
            clog(NetMessageSummary) << data.size() << " nodes known and returned;" << (numItemsToSend - data.size())
        ...
        pair<bytes, unsigned> receipts(RLP const &_blockHashes) const override
        {
            ...
            clog(NetMessageSummary) << n << " receipt lists known and returned;" << (numItemsToSend - n) << " unknown;"
                                    << (count > c_maxReceipts ? count - c_maxReceipts : 0) << " ignored";
...
EthereumHost::EthereumHost(BlockChain const &_ch, OverlayDB const &_db, TransactionQueue &_tq, BlockQueue &_bq,
                           u256 _networkId) :
    ...
    foreachPeer([&](shared_ptr<EthereumPeer> _p)
                {
                    ...
                    if (n || _p->m_requireTransactions)
                    {
                        ...
                        _p->sealAndSend(ts);
                        clog(EthereumHostTrace) << "Sent" << n << "transactions to "
...
void EthereumHost::foreachPeer(std::function<bool(std::shared_ptr<EthereumPeer>)> const &_f) const
{
    ...
    auto sessionLess = [](std::pair<std::shared_ptr<SessionFace>, std::shared_ptr<Peer>> const &_left,
                          std::pair<std::shared_ptr<SessionFace>, std::shared_ptr<Peer>> const &_right)
    {
        return _left.first->rating() == _right.first->rating() ? _left.first->connectionTime() <
                                                                 _right.first->connectionTime() :
...
tuple<vector<shared_ptr<EthereumPeer>>, vector<shared_ptr<EthereumPeer>>, vector<shared_ptr<SessionFace>>>
    ...
    if (detailsFrom.totalDifficulty < detailsTo.totalDifficulty)
    {
        if (diff(detailsFrom.number, detailsTo.number) < 20)
        {
            ...
            for (shared_ptr<EthereumPeer> const &p: get<0>(s))
                ...
                {
                    RLPStream ts;
                    p->prep(ts, NewBlockPacket, 2).appendRaw(m_chain.block(b), 1).append(
                            m_chain.details(b).totalDifficulty);
...
EthereumHost::newPeerCapability(shared_ptr<SessionFace> const &_s, unsigned _idOffset, p2p::CapDesc const &_cap,
                                uint16_t _capID)
{
