...
Account *State::account(Address const &_addr)
{
    ...
    auto i = m_cache.emplace(
            ...
            std::forward_as_tuple(state[0].toInt<u256>(), state[1].toInt<u256>(), state[2].toHash<h256>(),
...
void State::clearCacheIfTooLarge() const
{
    ...
    while (m_unchangedCacheEntries.size() > 1000)
    {
        ...
        size_t const randomIndex = boost::random::uniform_int_distribution<size_t>(0,
                                                                                   m_unchangedCacheEntries.size() - 1)(
...
State::execute(EnvInfo const &_envInfo, SealEngineFace const &_sealEngine, Transaction const &_t, Permanence _p,
