...
namespace dev
{
    ...
    namespace eth
    {
        ...
        struct ClientWatch
        {
            ...
            explicit ClientWatch(h256 _id, Reaping _r) : id(_id), lastPoll(
                    _r == Reaping::Automatic ? std::chrono::system_clock::now()
        ...
        struct WatchChannel : public LogChannel
        {
...
#define cwatch LogOutputStream<WatchChannel, true>()
        ...
        struct WorkInChannel : public LogChannel
        {
            static const char *name();

            static const int verbosity = 16;
        };

        struct WorkOutChannel : public LogChannel
        {
            static const char *name();
        ...
        struct WorkChannel : public LogChannel
        {
            static const char *name();
...
#define cworkout LogOutputStream<WorkOutChannel, true>()
        ...
        public:
            ...
            virtual std::pair<h256, Address>
            submitTransaction(TransactionSkeleton const &_t, Secret const &_secret) override;

            using Interface::submitTransaction;

            /// Makes the given call. Nothing is recorded into the state.
            virtual ExecutionResult
            call(Address const &_secret, u256 _value, Address _dest, bytes const &_data, u256 _gas, u256 _gasPrice,
                 BlockNumber _blockNumber, FudgeFactor _ff = FudgeFactor::Strict) override;

            using Interface::call;

            /// Makes the given create. Nothing is recorded into the state.
            virtual ExecutionResult
            create(Address const &_secret, u256 _value, bytes const &_data, u256 _gas, u256 _gasPrice,
                   BlockNumber _blockNumber, FudgeFactor _ff = FudgeFactor::Strict) override;
            ...
            virtual std::pair<u256, ExecutionResult>
            estimateGas(Address const &_from, u256 _value, Address _dest, bytes const &_data, int64_t _maxGas,
                        u256 _gasPrice, BlockNumber _blockNumber, GasEstimationCallback const &_callback) override;
            ...
            virtual void prependLogsFromBlock(LogFilter const &_filter, h256 const &_blockHash, BlockPolarity _polarity,
            ...
            virtual LocalisedTransactionReceipt
            localisedTransactionReceipt(h256 const &_transactionHash) const override;
            ...
            {
                BOOST_THROW_EXCEPTION(InterfaceNotSupported("ClientBase::startSealing"));
            }

            virtual void stopSealing() override
            {
                BOOST_THROW_EXCEPTION(InterfaceNotSupported("ClientBase::stopSealing"));
            }

            virtual bool wouldSeal() const override
            {
                BOOST_THROW_EXCEPTION(InterfaceNotSupported("ClientBase::wouldSeal"));
            }

            virtual SyncStatus syncStatus() const override
            {
                BOOST_THROW_EXCEPTION(InterfaceNotSupported("ClientBase::syncStatus"));
            }
        ...
        protected:
            ...
            std::unordered_map<h256, h256s> m_specialFilters = std::unordered_map<h256, std::vector<h256>>{{PendingChangedFilter, {}},
                                                                                                           {ChainChangedFilter,   {}}};
