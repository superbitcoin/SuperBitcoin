...
const char *BlockQueueTraceChannel::name()
{
    return EthOrange "▣ ▶";
}
...
ImportResult BlockQueue::import(bytesConstRef _block, bool _isOurs)
{
    ...
    if (bi.timestamp() > utcTime() && !_isOurs)
    {
        ...
        clog(BlockQueueTraceChannel) << "OK - queued for future [" << bi.timestamp() << "vs" << utcTime()
                                     << "] - will wait until" << buf;
        m_difficulty += bi.difficulty();
        bool unknown = !m_readySet.count(bi.parentHash()) && !m_drainingSet.count(bi.parentHash()) &&
    ...
    DEV_GUARDED(m_verification)
    {
        ...
        while (moreBad)
        {
            ...
            std::vector<VerifiedBlock> badVerified = m_verified.removeIf([this](VerifiedBlock const &_b)
                                                                         {
                                                                             return m_knownBad.count(
                                                                                     _b.verified.info.parentHash()) ||
                                                                                    m_knownBad.count(
                                                                                            _b.verified.info.hash());
                                                                         });
...
bool BlockQueue::invariants() const
{
    ...
    {
        std::stringstream s;
        s << "Failed BlockQueue invariant: m_readySet: " << m_readySet.size() << " m_verified: " << m_verified.count()
          << " m_unverified: " << m_unverified.count() << " m_verifying" << m_verifying.count();
