...
private:
    void doInitDAG(unsigned _n)
    {
        ...
        cout << "Initializing DAG for epoch beginning #" << (_n / 30000 * 30000) << " (seedhash " << seedHash.abridged()
    ...
    void doBenchmark(std::string _m, unsigned _warmupDuration = 15, unsigned _trialDuration = 3, unsigned _trials = 5)
    {
        ...
        f.onSolutionFound([&](EthashProofOfWork::Solution)
                          { return false; });
    ...
    void doFarm(std::string _m, string const &_remote, unsigned _recheckPeriod)
    {
        ...
        while (true)
            try
            {
                ...
                cnote << "  Ethash: "
                      << h256(EthashAux::eval(current.seedHash, current.headerHash, solution.nonce).value).hex();
                if (EthashAux::eval(current.seedHash, current.headerHash, solution.nonce).value < current.boundary)
                {
                    bool ok = rpc.eth_submitWork("0x" + toString(solution.nonce), "0x" + toString(current.headerHash),
