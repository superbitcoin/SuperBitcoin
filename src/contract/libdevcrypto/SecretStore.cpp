...
static js::mValue upgraded(string const &_s)
{
    ...
    unsigned version = ret.count("Version") ? stoi(ret["Version"].get_str()) : ret.count("version")
                                                                               ? ret["version"].get_int() : 0;
    if (version == 1)
    {
...
bytesSec SecretStore::decrypt(string const &_v, string const &_pass)
{
    ...
    } else if (o["kdf"].get_str() == "scrypt")
    {
        ...
        derivedKey = scrypt(_pass, fromHex(p["salt"].get_str()), p["n"].get_int(), p["r"].get_int(), p["p"].get_int(),
                            p["dklen"].get_int());
    } else
    {
    ...
    } else if (o.count("sillymac"))
    {
        ...
        h256 macExp = sha3(
                asBytes(o["sillymacjson"].get_str()) + derivedKey.ref().cropped(derivedKey.size() - 16).toBytes() +
                cipherText);
        if (mac != macExp)
        {
            cwarn << "Invalid key - MAC mismatch; expected" << toString(macExp) << ", got" << toString(mac);
