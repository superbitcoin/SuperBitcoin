...
namespace dev
{
    namespace eth
    {
        ...
        struct ABISerialiser<FixedHash<N>>
        {
            ...
            {
                static_assert(N <= 32, "Cannot serialise hash > 32 bytes."); static_assert(N > 0,
                                                                                           "Cannot serialise zero-length hash."); return
                        bytes(32 - N, 0) + _t.asBytes();
            }
        };
        ...
        {
            static bytes serialise(u256 const &_t)
            {
                return h256(_t).asBytes();
            }
        };
        ...
        {
            static bytes serialise(u160 const &_t)
            {
                return bytes(12, 0) + h160(_t).asBytes();
            }
        };
        ...
        struct ABIDeserialiser<FixedHash<N>>
        {
            ...
            {
                static_assert(N <= 32, "Parameter sizes must be at most 32 bytes."); FixedHash<N> ret;
