                        ...
                        function<string()> const &_pass, KDF _kdf)
{
    ...
    if (!store().recode(u, _newPass, [&]()
    { return getPassword(u, _pass); }, _kdf))
        return false;
...
Secret KeyManager::secret(h128 const &_uuid, function<string()> const &_pass, bool _usePasswordCache) const
{
    if (_usePasswordCache)
        ...
        { return getPassword(_uuid, _pass); }, _usePasswordCache));
    else
...
void
KeyManager::importExisting(h128 const &_uuid, string const &_info, string const &_pass, string const &_passwordHint)
{
    bytesSec key = m_store.secret(_uuid, [&]()
    { return _pass; });
    if (key.empty())
        return;
...
void KeyManager::importExisting(h128 const &_uuid, string const &_accountName, Address const &_address,
                                h256 const &_passHash, string const &_passwordHint)
{
