...
namespace dev
{
    namespace eth
    {
        ...
        static void badBlockInfo(BlockHeader const &_bi, string const &_err)
        {
            ...
            ss << c_border +
                  ("                     Guru Meditation #" + string(max<int>(0, 8 - bin.size()), '0') + bin + "." +
                   _bi.hash().abridged() + "                    ") + c_border << endl;
            ss << c_space << endl;
            ss << c_line;
            cwarn << "\n" + ss.str();
        }
        ...
        string TransactionSkeleton::userReadable(bool _toProxy, function<pair<bool, string>(
                                                 ...
                                                 function<string(Address const &)> const &_formatAddress) const
        {
            if (creation)
            {
                ...
                return string("ÐApp is attempting to create a contract; ") +
                       ...
                       "to be endowed with " + formatBalance(value) + ", with additional network fees of up to " +
            ...
            if (!isContract)
            {
                ...
                return "ÐApp is attempting to send " + formatBalance(value) + " to a recipient " + _formatAddress(to) +
                       (_toProxy ? " (this transaction is not executed directly, but forwarded to another ÐApp)" : "") +
