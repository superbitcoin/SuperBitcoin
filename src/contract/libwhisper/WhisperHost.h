    ...
    namespace shh
    {
        ...
        public:
            ...
            virtual Topics const &fullTopics(unsigned _id) const override
            {
                try
                {
                    return m_filters.at(m_watches.at(_id).id).full;
                } catch (...)
                {
                    return EmptyTopics;
                }
            ...
            virtual h256s
            watchMessages(unsigned _watchId) override; ///< returns IDs of messages, which match specific watch criteria
            virtual Envelope envelope(h256 _m) const override
            {
                try
                {
                    dev::ReadGuard l(x_messages);
                    return m_messages.at(_m);
                } catch (...)
                {
                    return Envelope();
                }
