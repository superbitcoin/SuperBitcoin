...
namespace dev
{
    namespace shh
    {
        ...
        public:
            ...
            {
                inject(Message(_payload).seal(_topics, _ttl, _workToProve));
            }

            void post(Public _to, bytes const &_payload, Topics _topics, unsigned _ttl = 50, unsigned _workToProve = 50)
            {
                inject(Message(_payload).sealTo(_to, _topics, _ttl, _workToProve));
            }

            void post(Secret const &_from, bytes const &_payload, Topics _topics, unsigned _ttl = 50,
                      unsigned _workToProve = 50)
            {
                inject(Message(_payload).seal(_from, _topics, _ttl, _workToProve));
            }

            void post(Secret const &_from, Public _to, bytes const &_payload, Topics _topics, unsigned _ttl = 50,
                      unsigned _workToProve = 50)
            {
                inject(Message(_payload).sealTo(_from, _to, _topics, _ttl, _workToProve));
            }
        };

        struct WatshhChannel : public dev::LogChannel
        {
...
namespace dev
{
    namespace shh
    {
        ...
        public:
            ...
            h256s check()
            {
                return m_c ? m_c->checkWatch(m_id) : h256s();
            }

            h256s peek()
            {
                return m_c ? m_c->peekWatch(m_id) : h256s();
            }
