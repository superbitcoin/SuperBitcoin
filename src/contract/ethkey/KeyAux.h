...
{
    return getPassword("Enter passphrase for address " + keyManager.accountName(a) + " (" + a.abridged() + "; hint:" +
...
pair<string, string>
createPassword(KeyManager &_keyManager, std::string const &_prompt, std::string const &_pass = std::string(),
...
public:
    ...
    Secret getSecret(std::string const &_signKey)
    {
        ...
        else
        {
            if (h128 u = fromUUID(_signKey))
                return Secret(secretStore().secret(u, [&]()
                { return getPassword("Enter passphrase for key: "); }));
            if (_signKey.substr(0, 6) == "brain#" && _signKey.find(":") != string::npos)
                return KeyManager::subkey(KeyManager::brain(_signKey.substr(_signKey.find(":"))),
                                          stoul(_signKey.substr(6, _signKey.find(":") - 7)));
            ...
            if (a)
                ...
                { return getPassword("Enter passphrase for key (hint:" + keyManager().passwordHint(a) + "): "); });
    ...
    {
        return ICAP(_a).encoded() + " (" + _a.abridged() + ")";
    }
    ...
    void execute()
    {
        switch (m_mode)
        {
            ...
            case OperationMode::DecodeTx:
            {
                try
                {
                    TransactionBase t = m_inputs.empty() ? TransactionBase(m_toSign) : TransactionBase(
                            inputData(m_inputs[0]), CheckTransaction::None);
            ...
            case OperationMode::SignTx:
            {
                ...
                for (string const &i: m_inputs)
                {
                    ...
                    try
                    {
                        TransactionBase t = i.empty() ? TransactionBase(m_toSign) : TransactionBase(
                                inputData(i, &isFile), CheckTransaction::None);
                        if (m_forceNonce)
            ...
            case OperationMode::Inspect:
            {
                ...
                if (m_inputs.empty())
                    m_inputs.push_back(
                            toAddress(KeyManager::brain(getPassword("Enter brain wallet key phrase: "))).hex());
                for (auto i: m_inputs)
                {
                    ...
                    if (m_showSecret)
                    {
                        ...
                        cout << "  Secret: " << (m_showSecret ? toHex(s.ref()) : (toHex(s.ref().cropped(0, 8)) + "..."))
                             << endl;
                    }
                }
                break;
            ...
            case OperationMode::RecodeBare:
                for (auto const &i: m_inputs)
                    if (h128 u = fromUUID(i))
                        ...
                        { return getPassword("Enter passphrase for key " + toUUID(u) + ": "); }, kdf()))
            ...
            case OperationMode::New:
            {
                ...
                h128 u = usesMaster ? keyManager().import(k.secret(), m_name) : keyManager().import(k.secret(), m_name,
                                                                                                    m_lock, m_lockHint);
                cout << "Created key " << toUUID(u) << endl;
            ...
            case OperationMode::Import:
            {
                ...
                string pw;
                bytesSec s = keyManager().store().secret(u, [&]()
                { return (pw = getPassword("Enter the passphrase for the key: ")); });
                if (s.size() != 32)
                {
                ...
                for (auto const &i: m_inputs)
                    if (Address a = userToAddress(i))
                    {
                        ...
                        { return pw = getPassword(
                                "Enter old passphrase for key '" + i + "' (hint: " + keyManager().passwordHint(a) +
                                "): ");
                        });
                        if (!s)
                        {
                        ...
                        pair<string, string> np = createPassword(keyManager(),
                                                                 "Enter new passphrase for key '" + i + "': ");
                        if (keyManager().recode(a, np.first, np.second, [&]()
                        { return pw; }, kdf()))
                            cout << "Re-encoded key '" << i << "' successfully." << endl;
                        else
                            cerr << "Couldn't re-encode '" << i << "''; key corrupt or incorrect passphrase supplied."
                                 << endl;
                    } else
                        cerr << "Couldn't re-encode " << i << "; not found." << endl;
                break;
    ...
    {
        return m_lock.empty() ? createPassword("Enter a passphrase with which to secure account " + _accountName + ": ")
                              : m_lock;
    }
    ...
    static void streamHelp(ostream &_out)
    {
        _out
                ...
                << "    import [<uuid>|<file>|<secret-hex>] <name>  Import keys from given source and place in wallet."
                << endl
                << "    importpresale <file> <name>  Import a presale wallet into a key with the given name." << endl
                << "    importwithaddress [<uuid>|<file>|<secret-hex>] <address> <name>  Import keys from given source with given address and place in wallet."
                << endl
                << "    export [ <address>|<uuid> , ... ]  Export given keys." << endl
                << "    inspect [ <address>|<name>|<uuid>|<brainwallet> ] ...  Print information on the given keys."
                << endl
                ...
                << "WARNING: Brain wallets with human-generated passphrasses are highly susceptible to attack. Don't use such a thing for"
                << endl
                << "anything important." << endl
                << "    newbrain [ <name>|-- ]  Create a new 13-word brain wallet; argument is the name or if --, do not add to wallet."
                << endl
                ...
                << "    sign [ <address>|<uuid>|<file>|<brainwallet> ] ( [ <hex>|<file> , ... ] )  (Re-)Sign given transaction."
                << endl
...
private:
    ...
    KDF kdf() const
    {
        return m_kdf == "pbkdf2" ? KDF::PBKDF2_SHA256 : KDF::Scrypt;
    }
