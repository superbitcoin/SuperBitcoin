    ...
    if (_s != CheckNothingNew)
    {
        if (_bi.difficulty() < chainParams().u256Param("minimumDifficulty"))
            BOOST_THROW_EXCEPTION(InvalidDifficulty()
                                          << RequirementError(bigint(chainParams().u256Param("minimumDifficulty")),
                                                              bigint(_bi.difficulty())));

        if (_bi.gasLimit() < chainParams().u256Param("minGasLimit"))
            BOOST_THROW_EXCEPTION(InvalidGasLimit() << RequirementError(bigint(chainParams().u256Param("minGasLimit")),
                                                                        bigint(_bi.gasLimit())));

        if (_bi.gasLimit() > chainParams().u256Param("maxGasLimit"))
            BOOST_THROW_EXCEPTION(InvalidGasLimit() << RequirementError(bigint(chainParams().u256Param("maxGasLimit")),
                                                                        bigint(_bi.gasLimit())));
        ...
        if (daoHardfork != 0 && daoHardfork + 9 >= daoHardfork && _bi.number() >= daoHardfork &&
            ...
            if (_bi.extraData() != fromHex("0x64616f2d686172642d666f726b"))
                BOOST_THROW_EXCEPTION(ExtraDataIncorrect() << errinfo_comment(
    ...
    if (_parent)
    {
        ...
        if (
            ...
            BOOST_THROW_EXCEPTION(
                    InvalidGasLimit()
                            << errinfo_min((bigint)((bigint)parentGasLimit - (bigint)(parentGasLimit /
                                                                                      chainParams().u256Param(
                                                                                              "gasLimitBoundDivisor"))))
                            << errinfo_got((bigint)gasLimit)
                            << errinfo_max((bigint)((bigint)parentGasLimit +
                                                    parentGasLimit / chainParams().u256Param("gasLimitBoundDivisor")))
            );
    }
...
u256 Ethash::calculateDifficulty(BlockHeader const &_bi, BlockHeader const &_parent) const
{
    ...
    if (_bi.number() < chainParams().u256Param("homsteadForkBlock"))
        ...
        target = _bi.timestamp() >= _parent.timestamp() + durationLimit ? _parent.difficulty() - (_parent.difficulty() /
        ...
        target = _parent.difficulty() + _parent.difficulty() / 2048 *
                                        max<bigint>(1 - (bigint(_bi.timestamp()) - _parent.timestamp()) / 10, -99);
