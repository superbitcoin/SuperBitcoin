...
void RLPXHandshake::readAuthEIP8()
{
    ...
    clog(NetP2PConnect) << "p2p.connect.ingress receiving " << size << "bytes EIP-8 auth from "
...
void RLPXHandshake::readAckEIP8()
{
    ...
    clog(NetP2PConnect) << "p2p.connect.egress receiving " << size << "bytes EIP-8 ack from "
                        << m_socket->remoteEndpoint();
...
void RLPXHandshake::transition(boost::system::error_code _ech)
{
    ...
    m_idleTimer.async_wait([this, self](boost::system::error_code const &_ec)
                           {
                               if (!_ec)
                               {
                                   if (!m_socket->remoteEndpoint().address().is_unspecified())
                                       clog(NetP2PConnect) << "Disconnecting " << m_socket->remoteEndpoint()
                                                           << " (Handshake Timeout)";
                                   cancel();
                               }
                           });
    ...
    } else if (m_nextState == WriteHello)
    {
        ...
        clog(NetP2PConnect) << (m_originated ? "p2p.connect.egress" : "p2p.connect.ingress")
        ...
        ba::async_read(m_socket->ref(), boost::asio::buffer(m_handshakeInBuffer, handshakeSize),
                       [this, self](boost::system::error_code ec, std::size_t)
                       {
                           ...
                           else
                           {
                               ...
                               uint32_t frameSize =
                                       (uint32_t)(header[2]) | (uint32_t)(header[1]) << 8 | (uint32_t)(header[0]) << 16;
                               if (frameSize > 1024)
                               {
                                   // all future frames: 16777216
                                   clog(NetP2PWarn) << (m_originated ? "p2p.connect.egress" : "p2p.connect.ingress")
                               ...
                               bytes headerRLP(
                                       header.size() - 3 - h128::size);    // this is always 32 - 3 - 16 = 13. wtf?
                               ...
                               ba::async_read(m_socket->ref(),
                                              ...
                                              [this, self, headerRLP](boost::system::error_code ec, std::size_t)
                                              {
                                                  ...
                                                  else
                                                  {
                                                      ...
                                                      {
                                                          clog(NetTriviaSummary) << (m_originated ? "p2p.connect.egress"
                                                                                                  : "p2p.connect.ingress")
                                                      ...
                                                      {
                                                          clog(NetWarn) << "Handshake causing an exception:"
                                                                        << _e.what();
                                                          m_nextState = Error;
                                                          transition();
                                                      }
