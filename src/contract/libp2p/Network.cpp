...
int Network::tcp4Listen(bi::tcp::acceptor &_acceptor, NetworkPreferences const &_netPrefs)
{
    ...
    try
    {
        listenIP = _netPrefs.listenIPAddress.empty() ? bi::address_v4() : bi::address::from_string(
                _netPrefs.listenIPAddress);
    }
    catch (...)
    {
        cwarn << "Couldn't start accepting connections on host. Failed to accept socket on " << listenIP << ":"
              << _netPrefs.listenPort << ".\n" << boost::current_exception_diagnostic_information();
        return -1;
    }
                                       ...
                                       bi::address &o_upnpInterfaceAddr)
{
    ...
    if (upnp && upnp->isValid())
    {
        ...
        {
            clog(NetNote) << "Punched through NAT and mapped local port" << _listenPort << "onto external port"
                          << extPort << ".";
...
bi::tcp::endpoint Network::resolveHost(string const &_addr)
{
    ...
    else
    {
        ...
        if (ec)
        {
            clog(NetWarn) << "Error resolving host address..." << LogTag::Url << _addr << ":" << LogTag::Error
                          << ec.message();
