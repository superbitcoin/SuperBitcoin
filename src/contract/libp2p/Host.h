...
namespace dev
{
    ...
    namespace p2p
    {
        ...
        public:
            ...
            /// Create Peer and attempt keeping peer connected.
            void
            requirePeer(NodeID const &_node, bi::address const &_addr, unsigned short _udpPort, unsigned short _tcpPort)
            {
            ...
            {
                return "enode://" + id().hex() + "@" +
                       (networkPreferences().publicIPAddress.empty() ? m_tcpPublic.address().to_string()
                                                                     : networkPreferences().publicIPAddress) + ":" +
            ...
            {
                return NodeInfo(id(), (networkPreferences().publicIPAddress.empty() ? m_tcpPublic.address().to_string()
                                                                                    : networkPreferences().publicIPAddress),
        ...
        private:
            ...
            void
            run(boost::system::error_code const &error);            ///< Run network. Called serially via ASIO deadline timer. Manages connection state transitions.
