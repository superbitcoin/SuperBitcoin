...
const char *NodeTableWarn::name()
{
    return "!P!";
}

const char *NodeTableNote::name()
{
    return "*P*";
}

const char *NodeTableMessageSummary::name()
{
    return "-P-";
}

const char *NodeTableMessageDetail::name()
{
    return "=P=";
}

const char *NodeTableConnect::name()
{
    return "+P+";
}

const char *NodeTableEvent::name()
{
    return "+P+";
}

const char *NodeTableTimer::name()
{
    return "+P+";
}

const char *NodeTableUpdate::name()
{
    return "+P+";
}

const char *NodeTableTriviaSummary::name()
{
    return "-P-";
}

const char *NodeTableTriviaDetail::name()
{
    return "=P=";
}

const char *NodeTableAllDetail::name()
{
    return "=P=";
}

const char *NodeTableEgress::name()
{
    return ">>P";
}

const char *NodeTableIngress::name()
{
    return "<<P";
}

NodeEntry::NodeEntry(NodeID const &_src, Public const &_pubk, NodeIPEndpoint const &_gw) : Node(_pubk, _gw), distance(
        NodeTable::distance(_src, _pubk))
{
}
...
NodeTable::NodeTable(ba::io_service &_io, KeyPair const &_alias, NodeIPEndpoint const &_endpoint, bool _enabled) :
        ...
        m_socket(make_shared<NodeSocket>(_io, *reinterpret_cast<UDPSocketEvents *>(this),
                                         (bi::udp::endpoint)m_node.endpoint)),
...
shared_ptr<NodeEntry> NodeTable::addNode(Node const &_node, NodeRelation _relation)
{
    ...
    {
        DEV_GUARDED(x_nodes)
            clog(NodeTableConnect) << "Sending public key discovery Ping to" << (bi::udp::endpoint)_node.endpoint
...
void NodeTable::noteActiveNode(Public const &_pubk, bi::udp::endpoint const &_endpoint)
{
    ...
    {
        clog(NodeTableConnect) << "Noting active node:" << _pubk << _endpoint.address().to_string() << ":"
                               << _endpoint.port();
...
void NodeTable::onReceived(UDPSocketFace *, bi::udp::endpoint const &_from, bytesConstRef _packet)
{
    try
    {
        ...
        if (packet->isExpired())
        {
            clog(NodeTableWarn) << "Invalid packet (timestamp in the past) from " << _from.address().to_string() << ":"
                                << _from.port();
            return;
        }
    ...
    {
        clog(NodeTableWarn) << "Exception processing message from " << _from.address().to_string() << ":"
                            << _from.port() << ": " << _e.what();
    }
    catch (...)
    {
...
void NodeTable::doCheckEvictions()
{
    ...
    {
        if (_ec)
            clog(NodeTableMessageDetail) << "Check Evictions timer was probably cancelled: " << _ec.value()
                                         << _ec.message();
...
unique_ptr<DiscoveryDatagram> DiscoveryDatagram::interpretUDP(bi::udp::endpoint const &_from, bytesConstRef _packet)
{
    ...
    if (!sourceid)
    {
        clog(NodeTableWarn) << "Invalid packet (bad signature) from " << _from.address().to_string() << ":"
                            << _from.port();
        return decoded;
    }
    switch (signedBytes[0])
    {
        ...
        case Neighbours::type:
            ...
            break;
        default:
            clog(NodeTableWarn) << "Invalid packet (unknown packet type) from " << _from.address().to_string() << ":"
                                << _from.port();
            return decoded;
    }
