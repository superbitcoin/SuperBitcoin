...
namespace dev
{
    namespace p2p
    {
        ...
        struct RLPXWarn : public LogChannel
        {
        ...
        struct RLPXNote : public LogChannel
        {
            static const char *name();
        ...
        struct RLPXDatagramFace : public UDPDatagram
        {
            static uint32_t futureFromEpoch(std::chrono::seconds _sec)
            {
                return static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::seconds>(
                        (std::chrono::system_clock::now() + _sec).time_since_epoch()).count());
            }
        ...
        public:
            ...
            UDPSocket(ba::io_service &_io, UDPSocketEvents &_host, bi::udp::endpoint _endpoint) : m_host(_host),
            ...
            UDPSocket(ba::io_service &_io, UDPSocketEvents &_host, unsigned _port) : m_host(_host),
        ...
        void UDPSocket<Handler, MaxDatagramSize>::doRead()
        {
            ...
            m_socket.async_receive_from(boost::asio::buffer(m_recvData), m_recvEndpoint,
                                        [this, self](boost::system::error_code _ec, size_t _len)
                                        {
                                            ...
                                            if (_ec != boost::system::errc::success)
                                                clog(NetWarn) << "Receiving UDP message failed. " << _ec.value() << ":"
                                                              << _ec.message();

                                            if (_len)
                                                m_host.onReceived(this, m_recvEndpoint,
        ...
        void UDPSocket<Handler, MaxDatagramSize>::doWrite()
        {
            ...
            m_socket.async_send_to(boost::asio::buffer(datagram.data), endpoint,
                                   [this, self, endpoint](boost::system::error_code _ec, std::size_t)
                                   {
                                       ...
                                       if (_ec != boost::system::errc::success)
                                           clog(NetWarn) << "Failed delivering UDP message. " << _ec.value() << ":"
                                                         << _ec.message();
