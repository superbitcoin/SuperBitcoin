...
RLPXSocketIO::RLPXSocketIO(unsigned _protCount, RLPXFrameCoder &_coder, bi::tcp::socket &_socket, bool _flowControl,
...
void RLPXSocketIO::deferWrite()
{
    ...
    m_congestion->async_wait([=](boost::system::error_code const &_ec)
                             {
                                 m_congestion.reset();
                                 if (!_ec)
                                     doWrite();
...
void RLPXSocketIO::write(size_t _dequed)
{
    ...
    ba::async_write(m_socket, ba::buffer(m_toSend[0]),
                    [this, self, _dequed](boost::system::error_code ec, size_t written)
                    {
                        if (ec)
