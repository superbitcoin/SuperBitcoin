...
{
    setup(_init.m_originated, _init.m_remoteEphemeral, _init.m_remoteNonce, _init.m_ecdhe, _init.m_nonce,
...
RLPXFrameCoder::RLPXFrameCoder(bool _originated, h512 const &_remoteEphemeral, h256 const &_remoteNonce,
                               crypto::ECDHE const &_ecdhe, h256 const &_nonce, bytesConstRef _ackCipher,
...
void RLPXFrameCoder::writeFrame(uint16_t _protocolType, uint16_t _seqId, uint32_t _totalSize, bytesConstRef _payload,
                                bytes &o_bytes)
{
    RLPStream header;
    uint32_t len = (uint32_t)_payload.size();
