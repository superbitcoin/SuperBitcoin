                            ...
                            std::shared_ptr<RLPXSocket> const &_s)
{
    ...
    caps.erase(remove_if(caps.begin(), caps.end(), [&](CapDesc const &_r)
    {
    ...
    clog(NetMessageSummary) << "Hello: " << clientVersion << "V[" << protocolVersion << "]" << _id << showbase
                            << capslog.str() << dec << listenPort;

    // create session so disconnects are managed
    shared_ptr<SessionFace> ps = make_shared<Session>(this, move(_io), _s, p, PeerSessionInfo(
            {_id, clientVersion, p->endpoint.address.to_string(), listenPort, chrono::steady_clock::duration(),
             _rlp[2].toSet<CapDesc>(), 0, map<string, string>(), protocolVersion}));
...
void Host::determinePublic()
{
    ...
    auto laddr = m_netPrefs.listenIPAddress.empty() ? bi::address() : bi::address::from_string(
            m_netPrefs.listenIPAddress);
    auto lset = !laddr.is_unspecified();
    auto paddr = m_netPrefs.publicIPAddress.empty() ? bi::address() : bi::address::from_string(
    ...
    } else if (m_netPrefs.traverseNAT)
    {
        ...
        ep = Network::traverseNAT(lset && ifAddresses.count(laddr) ? std::set<bi::address>({laddr}) : ifAddresses,
                                  m_listenPort, natIFAddr);

        if (lset && natIFAddr != laddr)
            // if listen address is set, Host will use it, even if upnp returns different
            clog(NetWarn) << "Listen address" << laddr << "differs from local address" << natIFAddr
                          << "returned by UPnP!";

        if (pset && ep.address() != paddr)
        {
            // if public address is set, Host will advertise it, even if upnp returns different
            clog(NetWarn) << "Specified public address" << paddr << "differs from external address" << ep.address()
                          << "returned by UPnP!";
            ep.address(paddr);
        }
    } else if (pset)
...
void Host::runAcceptor()
{
    ...
    if (m_run && !m_accepting)
    {
        ...
        m_tcp4Acceptor.async_accept(socket->ref(), [=](boost::system::error_code ec)
        {
            ...
            {
                clog(NetConnect) << "Dropping incoming connect due to maximum peer count (" << Ingress
...
bytes Host::saveNetwork() const
{
    ...
    for (auto const &p: peers)
    {
        ...
        if (chrono::system_clock::now() - p.m_lastConnected < chrono::seconds(3600 * 48) && !!p.endpoint &&
            p.id != id() && (p.peerType == PeerType::Required || p.endpoint.isAllowed()))
        {
...
void Host::restoreNetwork(bytesConstRef _b)
{
    ...
    if (r.itemCount() > 0 && r[0].isInt() && fileVersion >= dev::p2p::c_protocolVersion - 1)
    {
        ...
        for (auto i: r[2])
        {
            ...
            {
                Node n((NodeID)i[2], NodeIPEndpoint(bi::address_v4(i[0].toArray<byte, 4>()), i[1].toInt<uint16_t>(),
                                                    i[1].toInt<uint16_t>()));
