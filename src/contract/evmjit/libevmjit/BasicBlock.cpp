...
namespace dev
{
    namespace eth
    {
        namespace jit
        {
            ...
            BasicBlock::BasicBlock(instr_idx _firstInstrIdx, code_iterator _begin, code_iterator _end,
                                   llvm::Function *_mainFunc) :
                    m_firstInstrIdx{_firstInstrIdx},
                    m_begin(_begin),
                    m_end(_end),
                    m_llvmBB(llvm::BasicBlock::Create(_mainFunc->getContext(), {".", std::to_string(_firstInstrIdx)},
                                                      _mainFunc))
            {
            }
                    ...
                    CompilerHelper(_builder)
            {
                ...
                m_sp = m_builder.CreateCall(getStackPrepareFunc(),
                                            {_runtimeManager.getStackBase(), _runtimeManager.getStackSize(), undef,
                                             undef, undef, _runtimeManager.getJmpBuf()},
            ...
            llvm::Value *LocalStack::get(size_t _index)
            {
                ...
                if (!item)
                {
                    ...
                    item = m_builder.CreateAlignedLoad(slot,
                                                       16); // TODO: Handle malloc alignment. Also for 32-bit systems.
            ...
            void LocalStack::finalize()
            {
                ...
                for (auto globalIdx = -static_cast<ssize_t>(m_input.size()); globalIdx < size(); ++globalIdx)
                {
                    ...
                    auto slot = m_builder.CreateConstGEP1_64(m_sp, globalIdx);
                    m_builder.CreateAlignedStore(item, slot,
                                                 16); // TODO: Handle malloc alignment. Also for 32-bit systems.
                }
            }
            ...
            llvm::Function *LocalStack::getStackPrepareFunc()
            {
                ...
                llvm::Type *argsTys[] = {Type::WordPtr, Type::Size->getPointerTo(), Type::Size, Type::Size, Type::Size,
                                         Type::BytePtr};
                ...
                auto maxOk = m_builder.CreateICmpULE(maxSize, m_builder.getInt64(RuntimeManager::stackSizeLimit),
                                                     "ok.max");
