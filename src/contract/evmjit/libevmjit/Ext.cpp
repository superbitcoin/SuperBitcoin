    ...
    namespace eth
    {
        namespace jit
        {
            ...
            namespace
            {
                ...
                llvm::FunctionType *
                getFunctionType(llvm::Type *_returnType, std::initializer_list<llvm::Type *> const &_argsTypes)
                {
                    return llvm::FunctionType::get(_returnType,
                                                   llvm::ArrayRef<llvm::Type *>{_argsTypes.begin(), _argsTypes.size()},
                                                   false);
                }
                ...
                llvm::Function *createFunc(EnvFunc _id, llvm::Module *_module)
                {
                    ...
                    return llvm::Function::Create(std::get<1>(desc), llvm::Function::ExternalLinkage, std::get<0>(desc),
                                                  _module);
                }
                ...
                llvm::Function *getQueryFunc(llvm::Module *_module)
                {
                    ...
                    if (!func)
                    {
                        ...
                        auto fty = llvm::FunctionType::get(Type::Void,
                                                           {Type::WordPtr, Type::EnvPtr, i32, Type::WordPtr}, false);
                ...
                llvm::Function *getUpdateFunc(llvm::Module *_module)
                {
                    ...
                    if (!func)
                    {
                        auto i32 = llvm::IntegerType::getInt32Ty(_module->getContext());
                        auto fty = llvm::FunctionType::get(Type::Void,
                                                           {Type::EnvPtr, i32, Type::WordPtr, Type::WordPtr}, false);
                        func = llvm::Function::Create(fty, llvm::Function::ExternalLinkage, funcName, _module);
                ...
                llvm::Function *getCallFunc(llvm::Module *_module)
                {
                    ...
                    if (!func)
                    {
                        ...
                        auto fty = llvm::FunctionType::get(
                                Type::Gas,
                                {Type::EnvPtr, i32, Type::Gas, hash160Ty->getPointerTo(), Type::WordPtr, Type::BytePtr,
            ...
            llvm::Value *Ext::calldataload(llvm::Value *_idx)
            {
                ...
                auto idx = m_builder.CreateTrunc(m_builder.CreateSelect(idxValid, _idx, callDataSize), Type::Size,
                                                 "idx");
            ...
            llvm::Value *Ext::balance(llvm::Value *_address)
            {
                ...
                auto pResult = getArgAlloca();
                createCABICall(func,
                               {pResult, getRuntimeManager().getEnvPtr(), m_builder.getInt32(EVM_BALANCE), address});
            ...
            llvm::Value *Ext::exists(llvm::Value *_address)
            {
                ...
                auto address = Endianness::toBE(m_builder, _address);
                auto pResult = getArgAlloca();
                createCABICall(func, {pResult, getRuntimeManager().getEnvPtr(), m_builder.getInt32(EVM_ACCOUNT_EXISTS),
                                      address});
            ...
            llvm::Value *Ext::blockHash(llvm::Value *_number)
            {
                ...
                auto pResult = getArgAlloca();
                createCABICall(func,
                               {pResult, getRuntimeManager().getEnvPtr(), m_builder.getInt32(EVM_BLOCKHASH), _number});
                return Endianness::toNative(m_builder, m_builder.CreateLoad(pResult));
            }
            ...
            MemoryRef Ext::extcode(llvm::Value *_addr)
            {
                ...
                createCABICall(func, {vPtr, getRuntimeManager().getEnvPtr(), m_builder.getInt32(EVM_CODE_BY_ADDRESS),
                                      address});
            ...
            llvm::Value *Ext::extcodesize(llvm::Value *_addr)
            {
                ...
                auto vPtr = getArgAlloca();
                createCABICall(func,
                               {vPtr, getRuntimeManager().getEnvPtr(), m_builder.getInt32(EVM_CODE_SIZE), address});
