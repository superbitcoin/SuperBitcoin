    ...
    namespace eth
    {
        namespace jit
        {
            ...
            namespace
            {
                llvm::Twine getName(RuntimeData::Index _index)
                {
                    switch (_index)
                    {
                        ...
                        case RuntimeData::ApparentCallValue:
                            return "msg.value";
                    ...
                    m_codeEnd(_codeEnd)
            {
                ...
                auto mallocFunc = llvm::Function::Create(llvm::FunctionType::get(Type::WordPtr, {Type::Size}, false),
                                                         llvm::Function::ExternalLinkage, "malloc", getModule());
                mallocFunc->setDoesNotThrow();
                mallocFunc->setDoesNotAlias(0);

                m_stackBase = m_builder.CreateCall(mallocFunc, m_builder.getInt64(
                        Type::Word->getPrimitiveSizeInBits() / 8 * stackSizeLimit),
                ...
                auto extGasPtr = m_builder.CreateStructGEP(getRuntimeDataType(), getDataPtr(), RuntimeData::Index::Gas,
                                                           "msg.gas.ptr");
            ...
            llvm::Value *RuntimeManager::getCode()
            {
                ...
                if (!m_codePtr)
                    m_codePtr = m_builder.CreateGlobalStringPtr(
                            {reinterpret_cast<char const *>(m_codeBegin), static_cast<size_t>(m_codeEnd - m_codeBegin)},
                            "code");
                return m_codePtr;
            }
