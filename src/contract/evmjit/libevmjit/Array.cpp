...
namespace dev
{
    namespace eth
    {
        namespace jit
        {
            ...
            llvm::Value *LazyFunction::call(IRBuilder &_builder, std::initializer_list<llvm::Value *> const &_args,
            ...
            {
                auto func = llvm::Function::Create(llvm::FunctionType::get(Type::Void, m_array->getType(), false),
                                                   llvm::Function::PrivateLinkage, "array.free", getModule());
                func->setDoesNotThrow();
                func->setDoesNotCapture(1);

                auto freeFunc = llvm::Function::Create(llvm::FunctionType::get(Type::Void, Type::BytePtr, false),
            ...
            llvm::Function *Array::getReallocFunc()
            {
                ...
                auto reallocFunc = llvm::Function::Create(
                        llvm::FunctionType::get(Type::BytePtr, reallocArgTypes, false), llvm::Function::ExternalLinkage,
                        "realloc", getModule());
            ...
            llvm::Function *Array::createExtendFunc()
            {
                ...
                InsertPointGuard guard{m_builder};
                m_builder.SetInsertPoint(llvm::BasicBlock::Create(m_builder.getContext(), {}, func));
                auto dataPtr = m_builder.CreateBitCast(arrayPtr, Type::BytePtr->getPointerTo(),
                                                       "dataPtr");// TODO: Use byte* in Array
                auto sizePtr = m_builder.CreateStructGEP(getType(), arrayPtr, 1, "sizePtr");
                auto capPtr = m_builder.CreateStructGEP(getType(), arrayPtr, 2, "capPtr");
                auto data = m_builder.CreateLoad(dataPtr, "data");
                auto size = m_builder.CreateLoad(sizePtr, "size");
                auto extSize = m_builder.CreateNUWSub(newSize, size, "extSize");
                auto newData = m_reallocFunc.call(m_builder, {data, newSize},
                                                  "newData"); // TODO: Check realloc result for null
