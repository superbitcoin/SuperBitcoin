...
namespace dev
{
    namespace eth
    {
        namespace jit
        {
            ...
            Compiler::compile(code_iterator _begin, code_iterator _end, std::string const &_id)
            {
                auto module = llvm::make_unique<llvm::Module>(_id,
                                                              m_builder.getContext()); // TODO: Provide native DataLayout
                ...
                for (auto it = _basicBlock.begin(); it != _basicBlock.end(); ++it)
                {
                    ...
                    switch (inst)
                    {
                        ...
                        case Instruction::DIV:
                        {
                            ...
                            n = m_builder.CreateSelect(divByZero, Constant::get(1),
                                                       n); // protect against hardware signal
                        ...
                        case Instruction::SMOD:
                        {
                            ...
                            r = m_builder.CreateSelect(divByZero, Constant::get(0), r);
                            r = m_builder.CreateSelect(divByMinusOne, Constant::get(0),
                                                       r); // protect against undef i256.min / -1
                            stack.push(r);
                            break;
                        }
                        ...
                        case Instruction::SIGNEXTEND:
                        {
                            ...
                            auto negmask = m_builder.CreateXor(mask, llvm::ConstantInt::getAllOnesValue(Type::Word),
                                                               "negmask");
                        ...
                        case Instruction::JUMPI:
                        {
                            auto destIdx = llvm::MDNode::get(m_builder.getContext(),
                                                             llvm::ValueAsMetadata::get(stack.pop()));
                            ...
                            auto jumpInst = (inst == Instruction::JUMP) ?
                                            ...
                                            m_builder.CreateCondBr(
                                                    m_builder.CreateICmpNE(stack.pop(), Constant::get(0), "jump.check"),
