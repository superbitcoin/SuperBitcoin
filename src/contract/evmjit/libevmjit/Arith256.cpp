    ...
    namespace eth
    {
        namespace jit
        {
            ...
            llvm::Function *Arith256::getUDiv256Func(llvm::Module &_module)
            {
                ...
                auto func = llvm::Function::Create(llvm::FunctionType::get(Type::Word, {Type::Word, Type::Word}, false),
            ...
            namespace
            {
                ...
                {
                    auto udivremFunc =
                            _type == Type::Word ? Arith256::getUDivRem256Func(_module) : Arith256::getUDivRem512Func(
                                    _module);
            ...
            llvm::Function *Arith256::getSDivRem256Func(llvm::Module &_module)
            {
                ...
                auto func = llvm::Function::Create(llvm::FunctionType::get(retType, {Type::Word, Type::Word}, false),
                                                   llvm::Function::PrivateLinkage, funcName, &_module);
                func->setDoesNotThrow();
                func->setDoesNotAccessMemory();
            ...
            llvm::Function *Arith256::getExpFunc()
            {
                if (!m_exp)
                {
                    ...
                    auto eOdd = m_builder.CreateICmpNE(m_builder.CreateAnd(e, Constant::get(1)), Constant::get(0),
                                                       "e.isodd");
...
EXPORT void debug(uint64_t a, uint64_t b, uint64_t c, uint64_t d, char z)
{
    DLOG(JIT) << "DEBUG " << std::dec << z << ": " //<< d << c << b << a
              << " [" << std::hex << std::setfill('0') << std::setw(16) << d << std::setw(16) << c << std::setw(16) << b
              << std::setw(16) << a << "]\n";
}
}
