...
{
    KeyPair k = _km.presaleSecret(contentsString(_file), [&](bool)
    { return _pass(); });
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        } else if (arg == "--extra-data" && i + 1 < argc)
        {
            ...
            chainParams = ChainParams(genesisInfo(eth::Network::MainNetwork),
                                      genesisStateRoot(eth::Network::MainNetwork));
        ...
        else if (arg == "--peerset" && i + 1 < argc)
        {
            ...
            for (auto const &p: each)
            {
                ...
                try
                {
                    preferredNodes[publicKey] = make_pair(NodeIPEndpoint(bi::address::from_string(hostIP), port, port),
                                                          required);
                }
                catch (...)
                {
    ...
    auto netPrefs = publicIP.empty() ? NetworkPreferences(listenIP, listenPort, upnp) : NetworkPreferences(publicIP,
                                                                                                           listenIP,
                                                                                                           listenPort,
                                                                                                           upnp);
            ...
            if (!keyManager.load(masterPassword) && masterSet)
            {
                while (true)
                {
                    ...
                    cout <<
                         ...
                         getDataDir("ethereum") + "/keys.info" << endl;
                }
            }
        } else
        {
    ...
    if (testingMode)
        authenticator = [](TransactionSkeleton const &, bool) -> bool
        { return true; };
    else
    ...
    if (jsonRPCURL > -1 || ipc)
    {
        ...
        accountHolder.reset(new SimpleAccountHolder([&]()
                                                    { return web3.ethereum(); }, getAccountPassword, keyManager,
                                                    authenticator));
