...
static const int64_t MaxBlockGasLimit = ChainParams(genesisInfo(Network::MainNetwork)).u256Param(
...
int main(int argc, char **argv)
{
    ...
    unsigned total = 0;
    bigint memTotal;
    auto onOp = [&](uint64_t step, uint64_t PC, Instruction inst, bigint m, bigint gasCost, bigint gas, VM *vm,
                    ExtVMFace const *extVM)
    {
    ...
    if (mode == Mode::Statistics)
    {
        cout << "Gas used: " << res.gasUsed << " (+" << t.baseGasRequired(se->evmSchedule(envInfo))
             << " for transaction, -" << res.gasRefunded << " refunded)" << endl;
        ...
        for (auto const &c: {Instruction::SSTORE, Instruction::SLOAD, Instruction::CALL, Instruction::CREATE,
