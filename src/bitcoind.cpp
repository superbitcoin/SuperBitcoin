...
void InitPromOptions(bpo::options_description *app, bpo::variables_map &vm, int argc, char **argv, HelpMessageMode mode)
{
    ...
    bpo::options_description helpGroup("Options:");
    helpGroup.add_options()
            ("help,h", "Print this help message and exit")
            ("?", "Print this help message and exit")
            ("version", strVersion.c_str())
            ("alertnotify", bpo::value<string>(), "Execute command when a relevant alert is received or we see a really long fork (%s in cmd is replaced by message)")
            ("blocknotify", bpo::value<string>(), "Execute command when the best block changes (%s in cmd is replaced by block hash)")
            ("blocksonly", bpo::value<string>(), "Whether to operate in a blocks only mode, default: no(choices: n, no, y, yes)")  // -help-debug
            ("assumevalid", bpo::value<string>(), strprintf(_("If this block is in the chain assume that it and its ancestors are valid and potentially skip their script verification (0 to verify all, default: %s, testnet: %s)"), defaultChainParams->GetConsensus().defaultAssumeValid.GetHex(), testnetChainParams->GetConsensus().defaultAssumeValid.GetHex()).c_str())
            ...
            ("reindex", bpo::value<string>(), "Rebuild chain state and block index from the blk*.dat files on disk")

#ifndef WIN32
            ("sysperms", bpo::value<string>(), "Create new files with system default permissions, instead of umask 077, only effective with disabled wallet functionality (choices: n, no, y, yes)")
#endif

            ("txindex", bpo::value<string>(), "Maintain a full transaction index, used by the getrawtransaction rpc call");
    app->add(helpGroup);
    ...
    connGroup.add_options()
            ...
            ("discover", bpo::value<string>(), "Discover own IP addresses default: yes when listening and no -externalip or -proxy(choices: n, no, y, yes)")
            ("dns", bpo::value<string>(), "Allow DNS lookups for -addnode, -seednode and -connect, default: yes(choices: n, no, y, yes)")
            ("dnsseed", bpo::value<string>(), "Query for peer addresses via DNS lookup, if low on addresses default: yes unless -connect used(choices: n, no, y, yes)")
            ("externalip", bpo::value< vector<string> >(), "Specify your own public address")
            ("forcednsseed", bpo::value<string>(), "Always query for peer addresses via DNS lookup default: no(choices: n, no, y, yes)")
            ("listen", bpo::value<string>(), "Accept connections from outside default: yes if no -proxy or -connect(choices: n, no, y, yes)")
            ("listenonion", bpo::value<string>(), "Automatically create Tor hidden service default: yes(choices: n, no, y, yes)")
            ...
            ("permitbaremultisig", bpo::value<string>(), "Relay non-P2SH multisig(choices: n, no, y, yes)")
            ("peerbloomfilters", bpo::value<string>(), "Support filtering of blocks and transaction with bloom filters(choices: n, no, y, yes)")
            ...
            ("proxyrandomize", bpo::value<string>(), "Randomize credentials for every proxy connection. This enables Tor stream isolation(choices: n, no, y, yes)")
...
#ifdef USE_UPNP
    #if USE_UPNP
            ("upnp", bpo::value<string>()->default_value("yes"), "Use UPnP to map the listening port default: yes when listening and no -proxy(choices: n, no, y, yes)")
#else
            ("upnp", bpo::value<string>()->default_value("no"), "Use UPnP to map the listening port default: no when listening and no -proxy(choices: n, no, y, yes)")
#endif
#endif
    ...
    walletGroup.add_options()
            ("disablewallet", bpo::value<string>(), "Do not load the wallet and disable wallet RPC calls(choices: n, no, y, yes)")
            ("keypool", bpo::value<unsigned int>(), "Set key pool size")
            ("fallbackfee", bpo::value<string>(), strprintf(_("A fee rate (in %s/kB) that will be used when fee estimation has insufficient data (default: %s)"),
                                                                              CURRENCY_UNIT, FormatMoney(DEFAULT_FALLBACK_FEE)).c_str())
            ("discardfee", bpo::value<string>(), strprintf(_("The fee rate (in %s/kB) that indicates your tolerance for discarding change by adding it to the fee (default: %s). "
                                                                     "Note: An output is discarded if it is dust at this rate, but we will always discard up to the dust relay fee and a discard fee above that is limited by the fee estimate for the longest target"),
                                                           CURRENCY_UNIT, FormatMoney(DEFAULT_DISCARD_FEE)).c_str())
            ("mintxfee", bpo::value<string>(), strprintf(_("Fees (in %s/kB) smaller than this are considered zero fee for transaction creation (default: %s)"),
                                                         CURRENCY_UNIT, FormatMoney(DEFAULT_TRANSACTION_MINFEE)).c_str())
            ("paytxfee", bpo::value<string>(), strprintf(_("Fee (in %s/kB) to add to transactions you send (default: %s)"),
                                                         CURRENCY_UNIT, FormatMoney(payTxFee.GetFeePerK())).c_str())
            ("rescan", bpo::value<string>(), "Rescan the block chain for missing wallet transactions on startup")
            ("salvagewallet", bpo::value<string>(), "Attempt to recover private keys from a corrupt wallet on startup")
            ("spendzeroconfchange", bpo::value<string>(), "Spend unconfirmed change when sending transactions(choices: n, no, y, yes)")
    ...
    walletDebugGroup.add_options()
            ("dblogsize", bpo::value<unsigned int>(), "Flush wallet database activity from memory to disk log every <n> megabytes")
            ("flushwallet", bpo::value<string>(), "Run a thread to flush wallet periodically(choices: n, no, y, yes)")
            ("privdb", bpo::value<string>(), "Sets the DB_PRIVATE flag in the wallet db environment(choices: n, no, y, yes)")
            ("walletrejectlongchains", bpo::value<string>(), "Wallet will not create transactions that violate mempool chain limits(choices: n, no, y, yes)");
    ...
    debugGroup.add_options()
            ...
            /********************************-help-debug begin*********************************************/
            ("checkblocks", bpo::value<signed int>(), strprintf(_("How many blocks to check at startup (default: %u, 0 = all)"), DEFAULT_CHECKBLOCKS).c_str())
            ("checklevel", bpo::value<unsigned int>(), strprintf(_("How thorough the block verification of -checkblocks is (0-4, default: %u)"), DEFAULT_CHECKLEVEL).c_str())
            ("checkblockindex", bpo::value<string>(), strprintf("Do a full consistency check for mapBlockIndex, setBlockIndexCandidates, chainActive and mapBlocksUnlinked occasionally. Also sets -checkmempool, default: %u(choices: n, no, y, yes)", defaultChainParams->DefaultConsistencyChecks()).c_str())
            ("checkmempool", bpo::value<int>(), strprintf("Run checks every <n> transactions (default: %u)", defaultChainParams->DefaultConsistencyChecks()).c_str())
            ...
            ("limitancestorsize", bpo::value<unsigned int>(), "Do not accept transactions whose size with all in-mempool ancestors exceeds <n> kilobytes")
            ("limitdescendantcount", bpo::value<unsigned int>(), "Do not accept transactions if any ancestor would have <n> or more in-mempool descendants")
            ("limitdescendantsize", bpo::value<unsigned int>(), "Do not accept transactions if any ancestor would have more than <n> kilobytes of in-mempool descendants")
            ("vbparams", bpo::value< vector<string> >()->multitoken(), "Use given start/end times for specified version bits deployment (regtest-only)")
            /*********************************-help-debug end**********************************************/
            ("debug", bpo::value< vector<string> >()->multitoken(), (". " +
                                                       _("If <category> is not supplied or if <category> = 1, output all debugging information.") + " " + _("<category> can be:") + " " + ListLogCategories() + ".").c_str())
            ("debugexclude", bpo::value< vector<string> >()->multitoken(), "Exclude debugging information for a category. Can be used in conjunction with -debug=1 to output debug logs for all categories except one or more specified categories.")
            ("-help-debug", bpo::value<string>(), "Show all debugging options, usage: --help -help-debug(choices: n, no, y, yes)")
            ("logips", bpo::value<string>(), "Include IP addresses in debug output(choices: n, no, y, yes)")
            ("logtimestamps", bpo::value<string>(), "Prepend debug output with timestamp(choices: n, no, y, yes)")
            /********************************-help-debug begin*********************************************/
            ("logtimemicros", bpo::value<string>(), "Add microsecond precision to debug timestamps(choices: n, no, y, yes)")
            ("mocktime", bpo::value<int64_t>(), "Replace actual time with <n> seconds since epoch (default: 0)")
            ("maxsigcachesize", bpo::value<unsigned int>(), "Limit sum of signature cache and script execution cache sizes to <n> MiB")
            ("maxtipage", bpo::value<int64_t>(), "Maximum tip age in seconds to consider node in initial block download")
            /*********************************-help-debug end**********************************************/
            ("maxtxfee", bpo::value<string>(), strprintf(_("Maximum total fees (in %s) to use in a single wallet transaction or raw transaction; setting this too low may abort large transactions (default: %s)"),
                                                                            CURRENCY_UNIT, FormatMoney(DEFAULT_TRANSACTION_MAXFEE)).c_str())
            ("printtoconsole", bpo::value<string>(), "Send trace/debug info to console instead of debug.log file(choices: n, no, y, yes)")
            ("printpriority", bpo::value<string>(), "Log transaction fee per kB when mining blocks(choices: n, no, y, yes)")  // -help-debug
            ("shrinkdebugfile", bpo::value<string>(), "Shrink debug.log file on client startup(choices: n, no, y, yes)");
    app->add(debugGroup);

    bpo::options_description chainSelectionGroup("Chain selection options:");
    chainSelectionGroup.add_options()
            ("testnet", bpo::value<string>(), "Use the test chain(choices: n, no, y, yes)")
            ("regtest", bpo::value<string>(), "Enter regression test mode, which uses a special chain in which blocks can be solved instantly. "
                    "This is intended for regression testing tools and app development(choices: n, no, y, yes).");
    app->add(chainSelectionGroup);

    bpo::options_description nodeRelayGroup("Node relay options:");
    nodeRelayGroup.add_options()
            /********************************-help-debug begin*********************************************/
            ("acceptnonstdtxn", bpo::value<string>(), strprintf("Relay and mine \"non-standard\" transactions (%sdefault: %u)", "testnet/regtest only; param choices: n, no, y, yes", defaultChainParams->RequireStandard()).c_str())
            ("incrementalrelayfee", bpo::value<string>(), strprintf("Fee rate (in %s/kB) used to define cost of relay, used for mempool limiting and BIP 125 replacement. (default: %s)", CURRENCY_UNIT, FormatMoney(DEFAULT_INCREMENTAL_RELAY_FEE)).c_str())
            ("dustrelayfee", bpo::value<string>(), strprintf("Fee rate (in %s/kB) used to defined dust, the value of an output such that it will cost more than its value in fees at this fee rate to spend it. (default: %s)", CURRENCY_UNIT, FormatMoney(DUST_RELAY_TX_FEE)).c_str())
            /*********************************-help-debug end**********************************************/
            ...
            ("whitelistrelay", bpo::value<string>(), "Accept relayed transactions received from whitelisted peers even when not relaying transactions(choices: n, no, y, yes)")
            ("whitelistforcerelay", bpo::value<string>(), "Force relay of transactions from whitelisted peers even if they violate local relay policy(choices: n, no, y, yes)");
    ...
    rpcServerGroup.add_options()
            ...
            ("rpcauth", bpo::value< vector<string> >()->multitoken(), _("Username and hashed password for JSON-RPC connections. The field <userpw> comes in the format: <USERNAME>:<SALT>$<HASH>. A canonical python script is included in share/rpcuser. The client then connects normally using the rpcuser=<USERNAME>/rpcpassword=<PASSWORD> pair of arguments. This option can be specified multiple times").c_str())
            ("rpcport", bpo::value<int>(), strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), defaultBaseParams->RPCPort(), testnetBaseParams->RPCPort()).c_str())
            ("rpcallowip", bpo::value< vector<string> >()->multitoken(), _("Allow JSON-RPC connections from specified source. Valid for <ip> are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24). This option can be specified multiple times").c_str())
            ("rpcserialversion", bpo::value<unsigned int>(), "Sets the serialization of raw transaction or block hex returned in non-verbose mode, non-segwit(0) or segwit(1)")
...
bool AppInit(int argc, char* argv[])
{
    ...
    gArgs.InitPromOptions(InitPromOptions, app, argc, argv, HMM_BITCOIND);
