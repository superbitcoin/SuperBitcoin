...
void InitPromOptions(bpo::options_description *app, bpo::variables_map &vm, int argc, const char **argv, HelpMessageMode mode)
{
    ...
    std::string strHead = strprintf(_("%s Daemon"), _(PACKAGE_NAME)) + " " + _("version") + " " + FormatFullVersion() + "\n" +
    ...
    helpGroup.add_options()
            ...
            ("blocksonly", bpo::value<string>(), "Whether to operate in a blocks only mode, default: no(parameters: n, no, y, yes)")  // -help-debug
...
#if HAVE_DECL_DAEMON
            ("daemon", bpo::value<string>(), "Run in the background as a daemon and accept commands(parameters: n, no, y, yes)") // dependence : mode, HAVE_DECL_DAEMON
#endif
#endif
            ...
            ("persistmempool", bpo::value<string>(), "Whether to save the mempool on shutdown and load on restart (parameters: n, no, y, yes)")
            ...
            ("prune", bpo::value<int32_t>(), strprintf(_("Reduce storage requirements by enabling pruning (deleting) of old blocks. This allows the pruneblockchain RPC to be called to delete specific blocks, and enables automatic pruning of old blocks if a target size in MiB is provided. This mode is incompatible with -txindex and -rescan. "
            ...
            ("reindex-chainstate", bpo::value<string>(), "Rebuild chain state from the currently indexed blocks (parameters: n, no, y, yes)")
            ("reindex", bpo::value<string>(), "Rebuild chain state and block index from the blk*.dat files on disk(parameters: n, no, y, yes)")

#ifndef WIN32
            ("sysperms", bpo::value<string>(), "Create new files with system default permissions, instead of umask 077, only effective with disabled wallet functionality (parameters: n, no, y, yes)")
#endif

            ("txindex", bpo::value<string>(), "Maintain a full transaction index, used by the getrawtransaction rpc call(parameters: n, no, y, yes)");
    app->add(helpGroup);
    ...
    walletGroup.add_options()
            ("disablewallet", bpo::value<string>(), "Do not load the wallet and disable wallet RPC calls(parameters:: n, no, y, yes)")
            ...
            ("rescan", bpo::value<string>(), "Rescan the block chain for missing wallet transactions on startup(parameters:: n, no, y, yes)")
            ("salvagewallet", bpo::value<string>(), "Attempt to recover private keys from a corrupt wallet on startup(parameters:: n, no, y, yes)")
            ("spendzeroconfchange", bpo::value<string>(), "Spend unconfirmed change when sending transactions(parameters:: n, no, y, yes)")
    ...
    walletDebugGroup.add_options()
            ...
            ("flushwallet", bpo::value<string>(), "Run a thread to flush wallet periodically(parameters:: n, no, y, yes)")
            ("privdb", bpo::value<string>(), "Sets the DB_PRIVATE flag in the wallet db environment(parameters:: n, no, y, yes)")
            ("walletrejectlongchains", bpo::value<string>(), "Wallet will not create transactions that violate mempool chain limits(parameters:: n, no, y, yes)");
    ...
    debugGroup.add_options()
            ...
            ("checkblockindex", bpo::value<string>(), strprintf("Do a full consistency check for mapBlockIndex, setBlockIndexCandidates, chainActive and mapBlocksUnlinked occasionally. Also sets -checkmempool, default: %u(parameters:: n, no, y, yes)", defaultChainParams->DefaultConsistencyChecks()).c_str())
            ...
            ("checkpoints", bpo::value<string>(), "Disable expensive verification for known chain history(parameters:: n, no, y, yes)")
            ("disablesafemode", bpo::value<string>(), "Disable safemode, override a real safe mode event(parameters:: n, no, y, yes)")
            ...
            ("stopafterblockimport", bpo::value<string>(), "Stop running after importing blocks from disk(parameters:: n, no, y, yes)")
            ...
            ("-help-debug", bpo::value<string>(), "Show all debugging options, usage: --help -help-debug(parameters:: n, no, y, yes)")
            ...
            /********************************-help-debug begin*********************************************/
            ("logtimemicros", bpo::value<string>(), "Add microsecond precision to debug timestamps(parameters:: n, no, y, yes)")
            ...
            ("printtoconsole", bpo::value<string>(), "Send trace/debug info to console instead of debug.log file(parameters:: n, no, y, yes)")
            ("printpriority", bpo::value<string>(), "Log transaction fee per kB when mining blocks(parameters:: n, no, y, yes)")  // -help-debug
            ("shrinkdebugfile", bpo::value<string>(), "Shrink debug.log file on client startup(parameters:: n, no, y, yes)");
    app->add(debugGroup);
    ...
    nodeRelayGroup.add_options()
            /********************************-help-debug begin*********************************************/
            ("acceptnonstdtxn", bpo::value<string>(), strprintf("Relay and mine \"non-standard\" transactions (%sdefault: %u)", "testnet/regtest only(parameters:: n, no, y, yes).", defaultChainParams->RequireStandard()).c_str())
            ...
            ("datacarrier", bpo::value<string>(), "Relay and mine data carrier transactions(parameters:: n, no, y, yes).")
            ("datacarriersize", bpo::value<unsigned int>(), "Maximum size of data in data carrier transactions we relay and mine")
            ("mempoolreplacement", bpo::value<string>(), "Enable transaction replacement in the memory pool(parameters:: n, no, y, yes).")
            ...
            ("whitelistrelay", bpo::value<string>(), "Accept relayed transactions received from whitelisted peers even when not relaying transactions(parameters:: n, no, y, yes)")
            ("whitelistforcerelay", bpo::value<string>(), "Force relay of transactions from whitelisted peers even if they violate local relay policy(parameters:: n, no, y, yes)");
