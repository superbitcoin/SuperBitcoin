...
UniValue getinfo(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
                "getinfo\n"
                        ...
                        "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in " + CURRENCY_UNIT +
                "/kB\n"
    ...
    obj.push_back(Pair("full_version",FormatFullVersion());
    ...
    obj.push_back(Pair("testnet", Params().NetworkIDString() == CChainParams::TESTNET));
...
UniValue getmemoryinfo(const JSONRPCRequest &request)
{
    ...
    std::string mode = (request.params.size() < 1 || request.params[0].isNull()) ? "stats"
