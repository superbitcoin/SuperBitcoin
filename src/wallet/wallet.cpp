...
bool
CWallet::ChangeWalletPassphrase(const SecureString &strOldWalletPassphrase, const SecureString &strNewWalletPassphrase)
{
...

    {
        ...
        for (MasterKeyMap::value_type &pMasterKey : mapMasterKeys)
        {
            ...
            if (CCryptoKeyStore::Unlock(_vMasterKey))
            {
                ...
                mlog_notice("Wallet passphrase changed to an nDeriveIterations of %i\n",
                            pMasterKey.second.nDeriveIterations);
...
bool CWallet::Verify()
{
    ...
    for (const std::string &walletFile : Args().GetArgs("-wallet"))
    {
        if (boost::filesystem::path(walletFile).filename() != walletFile)
        {
            return InitError(strprintf(
                    _("Error loading wallet %s. -wallet parameter must only specify a filename (not a path)."),
                    walletFile));
        }
        ...
        {
            return InitError(
                    strprintf(_("Error loading wallet %s. Invalid characters in -wallet filename."), walletFile));
        }

        fs::path wallet_path = fs::absolute(walletFile, Args().GetDataDir());

        if (fs::exists(wallet_path) && (!fs::is_regular_file(wallet_path) || fs::is_symlink(wallet_path)))
        {
            return InitError(
                    strprintf(_("Error loading wallet %s. -wallet filename must be a regular file."), walletFile));
        }

        if (!wallet_paths.insert(wallet_path).second)
        {
            return InitError(
                    strprintf(_("Error loading wallet %s. Duplicate -wallet filename specified."), walletFile));
        }

        std::string strError;
        if (!CWalletDB::VerifyEnvironment(walletFile, Args().GetDataDir().string(), strError))
        {
            return InitError(strError);
        }

        if (Args().GetArg<bool>("-salvagewallet", false))
        {
        ...
        bool dbV = CWalletDB::VerifyDatabaseFile(walletFile, Args().GetDataDir().string(), strWarning, strError);
        if (!strWarning.empty())
        {
...
bool CWallet::EncryptWallet(const SecureString &strWalletPassphrase)
{
    ...
    mlog_notice("Encrypting Wallet with an nDeriveIterations of %i\n", kMasterKey.nDeriveIterations);
...
bool CWallet::MarkReplaced(const uint256 &originalHash, const uint256 &newHash)
{
    ...
    {
        mlog_notice("%s: Updating walletdb tx %s failed", __func__, wtx.GetHash().ToString());
        success = false;
    }
 ...
 */
bool
CWallet::AddToWalletIfInvolvingMe(const CTransactionRef &ptx, const CBlockIndex *pIndex, int posInBlock, bool fUpdate)
{
        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx))
        {
            ...
            for (const CTxOut &txout: tx.vout)
            {
                ...
                for (const CKeyID &keyid : vAffected)
                {
                    ...
                    {
                        mlog_notice("%s: Detected a used keypool key, mark all keypool key up to this key as used\n",
                                    __func__);
                        MarkReserveKeysAsUsed(mi->second);

                        if (!TopUpKeyPool())
                        {
                            mlog_notice("%s: Topping up keypool failed (locked wallet)\n", __func__);
                        }
                    }
                }
            }
...
bool CWallet::SetHDMasterKey(const CPubKey &pubkey)
{
    ...
    // as a hdchain object
    CHDChain newHdChain;
    newHdChain.nVersion = CanSupportFeature(FEATURE_HD_SPLIT) ? CHDChain::VERSION_HD_CHAIN_SPLIT
                           ...
                           const isminefilter &filter) const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); ++i)
    {
        ...
        {
            mlog_notice("CWalletTx::GetAmounts: Unknown transaction type found, txid %s\n",
...
int64_t CWallet::RescanFromTime(int64_t startTime, bool update)
{
    ...
    mlog_notice("%s: Rescanning last %i blocks\n", __func__,
...
CBlockIndex *CWallet::ScanForWalletTransactions(CBlockIndex *pindexStart, bool fUpdate)
{
    ...
    CBlockIndex *ret = nullptr;
    {
        ...
        while (pindex && !fAbortRescan)
        {
            ...
            {
                nNow = GetTime();
                mlog_notice("Still rescanning. At block %d. Progress=%f\n", pindex->nHeight,
        ...
        {
            mlog_notice("Rescan aborted at block %d. Progress=%f\n", pindex->nHeight,
                        GuessVerificationProgress(chainParams.TxData(), pindex));
        }
...
void CWallet::ResendWalletTransactions(int64_t nBestBlockTime, CConnman *connman)
{
    ...
    if (!relayed.empty())
        mlog_notice("%s: rebroadcast %u unconfirmed transactions\n", __func__, relayed.size());
}
...
static void
ApproximateBestSubset(const std::vector<CInputCoin> &vValue, const CAmount &nTotalLower, const CAmount &nTargetValue,
                                 ...
                                 std::set<CInputCoin> &setCoinsRet, CAmount &nValueRet) const
{
    ...
    } else
    {
        ...
        {
            mlog_notice("SelectCoins() best subset: ");
            for (unsigned int i = 0; i < vValue.size(); i++)
            {
                if (vfBest[i])
                {
                    mlog_notice("%s ", FormatMoney(vValue[i].txout.nValue));
                }
            }
            mlog_notice("total %s\n", FormatMoney(nBest));
        }
...
bool CWallet::SelectCoins(const std::vector<COutput> &vAvailableCoins, const CAmount &nTargetValue,
                          std::set<CInputCoin> &setCoinsRet, CAmount &nValueRet, const CCoinControl *coinControl) const
{
...
bool CWallet::SignTransaction(CMutableTransaction &tx)
{
    ...
    for (const auto &input : tx.vin)
    {
        ...
        if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, amount, SIGHASH_ALL), scriptPubKey,
                              sigdata))
        {
            return false;
        }
...
static CFeeRate GetDiscardRate(const CBlockPolicyEstimator &estimator)
{
    ...
    CFeeRate discard_rate = estimator.estimateSmartFee(highest_target, nullptr /* FeeCalculation */,
                                                       false /* conservative */);
    // Don't let discard_rate be greater than longest possible fee estimate if we get a valid fee estimate
    discard_rate = (discard_rate == CFeeRate(0)) ? CWallet::m_discard_rate : std::min(discard_rate,
                                ...
                                bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        if (sign)
        {
            ...
            for (const auto &coin : setCoins)
            {
                ...
                if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, coin.txout.nValue,
                                                                  SIGHASH_ALL | SIGHASH_SBTC_FORK), scriptPubKey,
                                      sigdata))
                {
                    strFailReason = _("Signing transaction failed");
                    return false;
    ...
    mlog_notice(
            ...
            nFeeRet, nBytes, nFeeNeeded, feeCalc.returnedTarget, feeCalc.desiredTarget,
...
bool CWallet::CommitTransaction(CWalletTx &wtxNew, CReserveKey &reservekey, CConnman *connman, CValidationState &state)
{
    {
        LOCK2(cs_main, cs_wallet);
        mlog_notice("CommitTransaction:%s", wtxNew.tx->ToString());
        {
        ...
        if (fBroadcastTransactions)
        {
            ...
            {
                mlog_notice("CommitTransaction(): Transaction cannot be broadcast immediately, %s",
...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes, const CCoinControl &coin_control, const CTxMemPool &pool,
                               const CBlockPolicyEstimator &estimator, FeeCalculation *feeCalc)
{
    ...
    */
    ...
    { // 3. TODO: remove magic value of 0 for global payTxFee
...
{
    {
        ...
        mlog_notice("CWallet::NewKeyPool rewrote keypool");
    }
    return true;
}
...
{
    {
        ...
        {
            mlog_notice("keypool added %d keys (%d internal), size=%u (%u internal)", missingInternal + missingExternal,
...
void CWallet::ReturnKey(int64_t nIndex, bool fInternal, const CPubKey &pubkey)
{
    ...
    mlog_notice("keypool return %d", nIndex);
}
...
void CWallet::MarkReserveKeysAsUsed(int64_t keypool_id)
{
    ...
    while (it != std::end(*setKeyPool))
    {
        ...
        mlog_notice("keypool index %d removed\n", index);
...
void CWallet::GetKeyBirthTimes(std::map<CTxDestination, int64_t> &mapKeyBirth) const
{
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); it++)
    {
        ...
        const CWalletTx &wtx = (*it).second;
        CBlockIndex *pIndex = ifChainObj->GetBlockIndex(wtx.hashBlock);
...
unsigned int CWallet::ComputeTimeSmart(const CWalletTx &wtx) const
{
    ...
    {
        GET_CHAIN_INTERFACE(ifChainObj);
        if (ifChainObj->DoesBlockExist(wtx.hashBlock))
        {
            ...
            int64_t blocktime = (ifChainObj->GetBlockIndex(wtx.hashBlock))->GetBlockTime();
            nTimeSmart = std::max(latestEntry, std::min(blocktime, latestNow));
        } else
        {
            mlog_error("%s: found %s in block %s not in index\n", __func__, wtx.GetHash().ToString(),
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-discardfee=<amt>", strprintf(
...
CWallet *CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (Args().GetArg<int>("-zapwallettxes", 0))
    {
    ...
    if (Args().GetArg<int>("-upgradewallet", fFirstRun ? 1 : 0))
    {
        int nMaxVersion = Args().GetArg<int>("-upgradewallet", 0);
        if (nMaxVersion == 0) // the -upgradewallet without argument case
        {
            mlog_notice("Performing wallet upgrade to %i\n", FEATURE_LATEST);
            nMaxVersion = CLIENT_VERSION;
            walletInstance->SetMinVersion(FEATURE_LATEST); // permanently upgrade the wallet immediately
        } else
            mlog_notice("Allowing wallet upgrade up to %i\n", nMaxVersion);
    ...
    if (fFirstRun)
    {
        ...
        if (Args().GetArg<bool>("-usehd", DEFAULT_USE_HD_WALLET) && !walletInstance->IsHDEnabled())
        {
    ...
    {
        bool useHD = Args().GetArg<bool>("-usehd", DEFAULT_USE_HD_WALLET);
    ...
    if (!IsReScan())
    {
        ...
        {
            GET_CHAIN_INTERFACE(ifChainObj);
            pindexRescan = ifChainObj->FindForkInGlobalIndex(chainActive, locator);
        }
    }
    if (chainActive.Tip() && chainActive.Tip() != pindexRescan)
    {
        ...
        mlog_notice("Rescanning last %i blocks (from block %i)...", chainActive.Height() - pindexRescan->nHeight,
        ...
        mlog_notice(" rescan      %15dms\n", GetTimeMillis() - nStart);
        walletInstance->SetBestChain(chainActive.GetLocator());
        walletInstance->dbw->IncrementUpdateCounter();

        // Restore wallet transaction metadata after -zapwallettxes=1
        if (Args().GetArg<int>("-zapwallettxes", 0) && Args().GetArg<int>("-zapwallettxes", 1) != 2)
        {
            CWalletDB walletdb(*walletInstance->dbw);
...

    {
        ...
        mlog_notice("setKeyPool.size() = %u", walletInstance->GetKeyPoolSize());
        mlog_notice("mapWallet.size() = %u", walletInstance->mapWallet.size());
        mlog_notice("mapAddressBook.size() = %u", walletInstance->mapAddressBook.size());
    }
...
{
    bool blocksonly = Args().GetArg<bool>("-blocksonly", DEFAULT_BLOCKSONLY);
    bool walletbroadcast = Args().GetArg<bool>("-walletbroadcast", DEFAULT_WALLETBROADCAST);
    if (blocksonly && !Args().IsArgSet("-walletbroadcast"))
    {
        return false;
    }
...
bool CWallet::IsPersistmempool()
{
    ...
    bool persistmempool = Args().GetArg<bool>("-persistmempool", DEFAULT_PERSIST_MEMPOOL);
    // -zapwallettxes implies dropping the mempool on startup
    if (zapwallettxes != 0 && !Args().IsArgSet("-persistmempool"))
    {
        return false;
    }
...
bool CWallet::IsReScan()
{
    ...
    int zapwallettxes = Args().GetArg<int>("-zapwallettxes", 0);
    bool salvagewallet = Args().GetArg<bool>("-salvagewallet", false);
    bool reScan = Args().GetArg<bool>("-rescan", false);
    bool hasArgReScan = Args().IsArgSet("-rescan");
...
{
    vector<string> walletFiles = Args().GetArgs("-wallet");
...
bool CWallet::ParameterInteraction()
{
    ...
    if (is_multiwallet)
    {
        if (Args().GetArg<int>("-upgradewallet", 0))
        {
            return InitError(strprintf("%s is only allowed with a single wallet file", "-upgradewallet"));
        }
    }

    if (Args().GetArg<bool>("-sysperms", false))
        return InitError("-sysperms is not allowed in combination with enabled wallet functionality");
    if (Args().GetArg<int>("-prune", 0) && IsReScan())
        return InitError(
    ...
    {
        CAmount n = 0;
        if (!ParseMoney(Args().GetArg<std::string>("-mintxfee", std::string("")), n) || 0 == n)
            return InitError(AmountErrMsg("mintxfee", Args().GetArg<std::string>("-mintxfee", "")));
    ...
    {
        CAmount nFeePerK = 0;
        if (!ParseMoney(Args().GetArg<std::string>("-fallbackfee", std::string("")), nFeePerK))
            return InitError(strprintf(_("Invalid amount for -fallbackfee=<amount>: '%s'"),
                                       Args().GetArg<std::string>("-fallbackfee", "")));
    ...
    {
        CAmount nFeePerK = 0;
        if (!ParseMoney(Args().GetArg<std::string>("-discardfee", std::string("")), nFeePerK))
            return InitError(strprintf(_("Invalid amount for -discardfee=<amount>: '%s'"),
                                       Args().GetArg<std::string>("-discardfee", "")));
        if (nFeePerK > HIGH_TX_FEE_PER_KB)
    ...
    {
        CAmount nFeePerK = 0;
        if (!ParseMoney(Args().GetArg<std::string>("-paytxfee", std::string("")), nFeePerK))
            return InitError(AmountErrMsg("paytxfee", Args().GetArg<std::string>("-paytxfee", "")));
        if (nFeePerK > HIGH_TX_FEE_PER_KB)
        ...
        if (payTxFee < ::minRelayTxFee)
        {
            return InitError(strprintf(_("Invalid amount for -paytxfee=<amount>: '%s' (must be at least %s)"),
                                       Args().GetArg<std::string>("-paytxfee", ""), ::minRelayTxFee.ToString()));
        }
    }
    if (Args().IsArgSet("-maxtxfee"))
    {
        CAmount nMaxFee = 0;
        if (!ParseMoney(Args().GetArg<std::string>("-maxtxfee", std::string("")), nMaxFee))
            return InitError(AmountErrMsg("maxtxfee", Args().GetArg<std::string>("-maxtxfee", "")));
        ...
        if (CFeeRate(maxTxFee, 1000) < ::minRelayTxFee)
        {
            return InitError(strprintf(
                    ...
                    Args().GetArg<std::string>("-maxtxfee", ""), ::minRelayTxFee.ToString()));
        }
    }
    nTxConfirmTarget = Args().GetArg<uint32_t>("-txconfirmtarget", DEFAULT_TX_CONFIRM_TARGET);
    bSpendZeroConfChange = Args().GetArg<bool>("-spendzeroconfchange", DEFAULT_SPEND_ZEROCONF_CHANGE);
    fWalletRbf = Args().GetArg<bool>("-walletrbf", DEFAULT_WALLET_RBF);
