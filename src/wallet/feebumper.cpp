...
#include "rbf.h"
#include "block/validation.h" //for mempool access
        ...
        nNewFee(0)
{
    ...
    {
        vErrors.push_back(
                strprintf("Cannot bump transaction %s which was already bumped by transaction %s", txid.ToString(),
    ...
    } else
    {
        GET_TXMEMPOOL_INTERFACE(ifTxMempoolObj);
        CTxMemPool &mempool = ifTxMempoolObj->GetMemPool();
        nNewFee = CWallet::GetMinimumFee(maxNewTxSize, coin_control, mempool, ::feeEstimator,
