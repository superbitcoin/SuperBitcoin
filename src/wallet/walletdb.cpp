    ...
    while (true)
    {
        ...
        if (setRange)
            ssKey << std::make_pair(std::string("acentry"),
                                    std::make_pair((fAllAccounts ? std::string("") : strAccount), uint64_t(0)));
...
DBErrors CWalletDB::LoadWallet(CWallet *pwallet)
{
    ...
    try
    {
        ...
        if (!pcursor)
        {
            ELogFormat("Error getting wallet database cursor");
            return DB_CORRUPT;
        }
        ...
        while (true)
        {
            ...
            {
                ELogFormat("Error reading next record from wallet database");
                return DB_CORRUPT;
            }
    ...
    ILogFormat("nFileVersion = %d", wss.nFileVersion);

    ILogFormat("Keys: %u plaintext, %u encrypted, %u w/ metadata, %u total",
...
DBErrors CWalletDB::ZapSelectTx(std::vector<uint256> &vTxHashIn, std::vector<uint256> &vTxHashOut)
{
    ...
    for (uint256 hash : vTxHash)
    {
        ...
        } else if ((*it) == hash)
        {
            if (!EraseTx(hash))
            {
                ELogFormat("Transaction was found for deletion but returned database error: %s",
                         hash.GetHex());
                delerror = true;
            }
...
bool CWalletDB::RecoverKeysOnlyFilter(void *callbackData, CDataStream ssKey, CDataStream ssValue)
{
    ...
    if (!fReadOK)
    {
        ELogFormat("WARNING: CWalletDB::Recover skipping %s: %s", strType, strErr);
        return false;
    }
...
bool CWalletDB::VerifyDatabaseFile(const std::string &walletFile, const fs::path &dataDir, std::string &warningStr,
                                   std::string &errorStr)
{
