...
UniValue sendtoaddress(const JSONRPCRequest &request)
{
    ...
    CBitcoinAddress address(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid sbtc address");
        ...
        if (pwallet)
        {
            ...
            if (!ProduceSignature(DummySignatureCreator(pwallet), witscript, sigs) ||
                !VerifyScript(sigs.scriptSig, witscript, &sigs.scriptWitness,
                              MANDATORY_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_WITNESS_PUBKEYTYPE,
 ...
 */
void
ListTransactions(CWallet *const pwallet, const CWalletTx &wtx, const std::string &strAccount, int nMinDepth, bool fLong,
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
                "settxfee amount\n"
                        ...
                        "\nArguments:\n"
                        "1. amount         (numeric or string, required) The transaction fee in " + CURRENCY_UNIT +
                "/kB\n"
                        "\nResult\n"
...
UniValue listunspent(const JSONRPCRequest &request)
{
    ...
    pwallet->AvailableCoins(vecOutputs, !include_unsafe, nullptr, nMinimumAmount, nMaximumAmount, nMinimumSumAmount,
...
UniValue fundrawtransaction(const JSONRPCRequest &request)
{
            ...
            RPCTypeCheckObj(options,
                            {
                                    ...
                                    {"reserveChangeKey",       UniValueType(
                                            UniValue::VBOOL)}, // DEPRECATED (and ignored), should be removed in 0.16 or so.
    ...
    if (!pwallet->FundTransaction(tx, nFeeOut, changePosition, strFailReason, lockUnspents, setSubtractFeeFromOutputs,
                                  coinControl))
    {
        throw JSONRPCError(RPC_WALLET_ERROR, strFailReason);
    }
...
UniValue bumpfee(const JSONRPCRequest &request)
{
    ...
    if (!request.params[1].isNull())
    {
        ...
        { // TODO: alias this to conf_target
...
UniValue createcontract(const JSONRPCRequest& request){
    ...
    uint64_t blockGasLimit = sbtcDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(sbtcDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 6)
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                        ...
                        "3. gasPrice  (numeric or string, optional) gasPrice SBTC price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
    ...
    if (request.params.size() > 1){
        if(request.params[1].isNum()){
            ...
            nGasLimit = atoi64(request.params[1].get_str());
        }else{
    ...
    if (request.params.size() > 2){
        if(request.params[2].isNum()){
            ...
            double retval;
            if (!ParseDouble(request.params[2].get_str(), &retval))
    ...
    if (request.params.size() > 3){
        ...
        if (!senderAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid sbtc address to send from");
        else
            fHasSender=true;
    }
