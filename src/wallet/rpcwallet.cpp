...
UniValue addwitnessaddress(const JSONRPCRequest &request)
{
...

    {
        ...
        if (!IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()) &&
            !Args().GetArg<bool>("-walletprematurewitness", false))
        {
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 6)
        throw std::runtime_error(
                ...
                i64tostr(DEFAULT_GAS_LIMIT_OP_CREATE) + ", max: " + i64tostr(blockGasLimit) + "\n"
                        "3. gasPrice  (numeric or string, optional) gasPrice SBTC price per gas unit, default: " +
...
{
    if (Args().GetArg("-disablewallet", false))
        return;
