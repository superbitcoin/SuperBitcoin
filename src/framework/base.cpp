...
CApp::CApp(): nVersion(1), bShutdown(false), cArgs(new CArgsManager)
{
}
...
bool CApp::InitParams(int argc, char *argv[])
{
    ...
    if (!PreInit())
    {
        mlog.error("PreInit failed");
        return false;
    }

    if (!cArgs->Init(argc, argv))
    {
        mlog.error("cArgs->Init");
        return false;
    }
...
{
    return InitParams(argc, argv) && ForEachComponent(true, [](IComponent *component)
