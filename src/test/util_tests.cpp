...
BOOST_FIXTURE_TEST_SUITE(util_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(util_ParseHex)
    {
        ...
        BOOST_CHECK(
                result.size() == 4 && result[0] == 0x12 && result[1] == 0x34 && result[2] == 0x56 && result[3] == 0x78);

        // Leading space must be supported (used in CDBEnv::Salvage)
        result = ParseHex(" 89 34 56 78");
        BOOST_CHECK(
                result.size() == 4 && result[0] == 0x89 && result[1] == 0x34 && result[2] == 0x56 && result[3] == 0x78);
    ...
    BOOST_AUTO_TEST_CASE(util_DateTimeStrFormat)
    {
        ...
        BOOST_CHECK_EQUAL(DateTimeStrFormat("%a, %d %b %Y %H:%M:%S +0000", 1317425777),
    ...
    BOOST_AUTO_TEST_CASE(util_ParseParameters)
    {
        ...
        BOOST_CHECK(testArgs.GetOptionMap().count("string") && testArgs.GetOptionMap().count("integer") &&
                    testArgs.GetOptionMap().count("bool")
                    && !testArgs.GetOptionMap().count("f") && !testArgs.GetOptionMap().count("d"));

        BOOST_CHECK(testArgs.GetOptionMap()["string"].as<std::string>() == "str" &&
                    testArgs.GetOptionMap()["integer"].as<int>() == 1
...
#define E "check_postfix"
    ...
    BOOST_AUTO_TEST_CASE(strprintf_numbers)
    {
        ...
        BOOST_CHECK(strprintf("%s %u %s", B, u64t, E) == B
                            ...
                            E);
        BOOST_CHECK(strprintf("%s %x %s", B, u64t, E) == B
                            " ffffffffffffffff "
                            E);

        size_t st = 12345678; /* unsigned size_t test value */
        ssize_t sst = -12345678; /* signed size_t test value */
        BOOST_CHECK(strprintf("%s %d %s", B, sst, E) == B
                            " -12345678 "
                            E);
        BOOST_CHECK(strprintf("%s %u %s", B, st, E) == B
                            " 12345678 "
                            E);
        BOOST_CHECK(strprintf("%s %x %s", B, st, E) == B
                            " bc614e "
                            E);

        ptrdiff_t pt = 87654321; /* positive ptrdiff_t test value */
        ptrdiff_t spt = -87654321; /* negative ptrdiff_t test value */
        BOOST_CHECK(strprintf("%s %d %s", B, spt, E) == B
                            " -87654321 "
                            E);
        BOOST_CHECK(strprintf("%s %u %s", B, pt, E) == B
                            " 87654321 "
                            E);
        BOOST_CHECK(strprintf("%s %x %s", B, pt, E) == B
                            " 5397fb1 "
                            E);
    }
...
#undef E
    ...
    BOOST_AUTO_TEST_CASE(test_FormatParagraph)
    {
        ...
        BOOST_CHECK_EQUAL(FormatParagraph(
    ...
    BOOST_AUTO_TEST_CASE(test_FormatSubVersion)
    {
        ...
        comments2.push_back(SanitizeString(std::string("Comment2; .,_?@-; !\"#$%&'()*+/<=>[]\\^`{|}~"),
                                           SAFE_CHARS_UA_COMMENT)); // Semicolon is discouraged but not forbidden by BIP-0014
