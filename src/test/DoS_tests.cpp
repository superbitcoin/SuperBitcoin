...
BOOST_FIXTURE_TEST_SUITE(DoS_tests, TestingSetup)
    ...
    void AddRandomOutboundPeer(std::vector<CNode *> &vNodes, PeerLogicValidation &peerLogic)
    {
        ...
        vNodes.emplace_back(
                new CNode(id++, ServiceFlags(NODE_NETWORK | NODE_WITNESS), 0, INVALID_SOCKET, addr, 0, 0, CAddress(),
                          "", /*fInboundIn=*/ false));
    ...
    BOOST_AUTO_TEST_CASE(DoS_banscore)
    {
        ...
        connman->ClearBanned();
        gArgs.ForceSetArg("-banscore", (unsigned int)111); // because 11 is my favorite number
        CAddress addr1(ip(0xa0b0c001), NODE_NONE);
        ...
        peerLogic->SendMessages(&dummyNode1, interruptDummy);
        BOOST_CHECK(connman->IsBanned(addr1));
        gArgs.ForceSetArg("-banscore", DEFAULT_BANSCORE_THRESHOLD);
