...
public:
    ...
    SbtcDGP(SbtcState* _state, bool _dgpevm = true) : dgpevm(_dgpevm), state(_state) { initDataEIP158(); }
...
private:
    ...
    const SbtcState* state;

    dev::Address templateContract;

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageDGP; //sbtc hashedKey,<key,value>

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageTemplate;//sbtc hashedKey,<key,value>

    std::vector<unsigned char> dataTemplate;   //sbtc call contract ,res->output
