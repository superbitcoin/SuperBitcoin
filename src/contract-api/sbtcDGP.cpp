...
bool SbtcDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule SbtcDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t SbtcDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t SbtcDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t SbtcDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t SbtcDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool SbtcDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void SbtcDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void SbtcDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void SbtcDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void SbtcDGP::createParamsInstance(){
...
dev::Address SbtcDGP::getAddressForBlock(unsigned int blockHeight){
...
void SbtcDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SbtcDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SbtcDGP::parseStorageOneUint64(uint64_t& value){
...
void SbtcDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule SbtcDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
